...
public class FinancialStatusService {
    ...
    private static final String FINANCIAL_STATUSES="financialstatuses";

    private static final String FINANCIAL_STATUS="financialStatus";
                ...
                try {
                    ...
                    case Constants.ACTION_CREATE:
                        if (financialstatuses == null) {
                            throw new InvalidDataException(FINANCIAL_STATUSES, ErrorCode.NOT_NULL.getCode(),
                                    null);
                        }
                        for (FinancialStatus financialStatus : financialstatuses) {
                            ...
                            if (!financialStatusRepository.uniqueCheck("name", financialStatus)) {
                                errors.addError(new FieldError(FINANCIAL_STATUS , "name", financialStatus.getName(), false,
                                        new String[] { ErrorCode.NON_UNIQUE_VALUE.getCode() }, null, null));
                            }
                            if (!financialStatusRepository.uniqueCheck("code", financialStatus)) {
                                errors.addError(new FieldError(FINANCIAL_STATUS, "code", financialStatus.getCode(), false,
                                        new String[] { ErrorCode.NON_UNIQUE_VALUE.getCode() }, null, null));
                            }
                        }
                        break;
