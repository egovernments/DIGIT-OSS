...
public class CollectionNotificationConsumer{
    ...
    private String buildSmsBody(Bill bill, PaymentDetail paymentDetail, RequestInfo requestInfo, String paymentStatus){
        ...
        if(!StringUtils.isEmpty(content)){
            ...
            content = content.replace("{rcpt_link}", receiptLink);
            ...
            if(content.contains("{amount_paid}"))
                content = content.replace("{amount_paid}", paymentDetail.getTotalAmountPaid().toString());

            content = content.replace("{rcpt_no}", paymentDetail.getReceiptNumber());
            content = content.replace("{mod_name}", moduleName);
            content = content.replace("{unique_id}", bill.getConsumerCode());
            message = content;
        }
        return message;
    }
