...
import com.tarento.analytics.org.service.MdmsService;
        ...
        try {
            ...
            nodes.forEach(node -> {
                if(!values.contains(node.asText()))
                    replaceField(responseNode, Constants.MDMSKeys.KEY, node.asText(),mdmsService.getDDRNameByCode(node.asText()));
