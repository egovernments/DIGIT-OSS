...
public class BoundaryControllerTest {
	...
	private String string1="/boundarys";

	private String string2="tenantId";

	private String string3="default";

	private String string4="boundarySearchResponse.json";

	private String string5="/boundarys/getLocationByLocationName";

	private String string6="Bank Road";

	private String string7="ap.public";

	private String string8="boundaryResponse.json";

	private String string9="X-CORRELATION-ID";

	private String string10="someId";

	private String string11="/boundarys/_search";
	...
	public void testShouldCreateBoundary() throws Exception {
		...
		mockMvc.perform(post(string1).contentType(MediaType.APPLICATION_JSON_UTF8)
	...
	public void testShouldUpdateBoundary() throws Exception {
		...
		when(boundaryTypeService.findByTenantIdAndCode(any(String.class), any(String.class))).thenReturn(boundaryType);
		mockMvc.perform(put("/boundarys/TEST").param(string2, string3).contentType(MediaType.APPLICATION_JSON_UTF8)
				.content(getFileContents("boundaryCreateRequest.json"))).andExpect(status().isCreated())
				.andExpect(content().contentType(MediaType.APPLICATION_JSON_UTF8))
				.andExpect(content().json(getFileContents("boundaryCreateResponse.json")));
	}
	...
	public void testShouldGetBoundaries() throws Exception {
		...
		mockMvc.perform(get(string1).param("Boundary.tenantId", string3).contentType(MediaType.APPLICATION_JSON_UTF8)
				.content(getFileContents(string4))).andExpect(status().isOk())
				.andExpect(content().contentType(MediaType.APPLICATION_JSON_UTF8))
				.andExpect(content().json(getFileContents(string4)));
	}
	
	@Test
	public void testShouldNotGetBoundariesWhenTenantIdIsNotThere() throws Exception {
		when(boundaryService.getAllBoundary(any(BoundaryRequest.class))).thenReturn(getSearchBoundaries());
		mockMvc.perform(get(string1).contentType(MediaType.APPLICATION_JSON_UTF8)).andExpect(status().isBadRequest());
	...
	public void testShouldGetLocationByLocationName() throws Exception {
		...
		mockMvc.perform(get(string5).param("locationName", "Appikonda Veedhi").param(string2, string3).contentType(MediaType.APPLICATION_JSON_UTF8)
	...
	public void testShouldNotGetLocationByLocationNameWhenLocationNameIsNotThere() throws Exception {
		when(boundaryService.getBoundaryDataByTenantIdAndNameLike(any(String.class),any(String.class))).thenReturn(getBoundariesByLocationByLocationName());
		mockMvc.perform(get(string5).param(string2, string3).contentType(MediaType.APPLICATION_JSON_UTF8)).andExpect(status().isBadRequest());
	}
	
	@Test
	public void testShouldNotGetLocationByLocationNameWhenTenantIsNotThere() throws Exception {
		when(boundaryService.getBoundaryDataByTenantIdAndNameLike(any(String.class),any(String.class))).thenReturn(getBoundariesByLocationByLocationName());
		mockMvc.perform(get(string5).param("locationName", "Appikonda Veedhi").contentType(MediaType.APPLICATION_JSON_UTF8)).andExpect(status().isBadRequest());
	}

	@Test
	public void testShouldFetchAllLocationsForGivenWard() throws Exception {
		final Boundary expectedBoundary = Boundary.builder().id(1L).name(string6).build();
		...
		mockMvc.perform(
				post("/boundarys/childLocationsByBoundaryId").param("boundaryId", "1").param(string2, string7))
				.andExpect(status().isOk()).andExpect(content().contentType(MediaType.APPLICATION_JSON_UTF8))
				.andExpect(content().json(getFileContents(string8)));
	}
	...
	public void testShouldReturnBadRequestWhenWardIsEmpty() throws Exception {
		when(crossHierarchyService.getActiveChildBoundariesByBoundaryIdAndTenantId(any(Long.class), any(String.class)))
				.thenReturn(null);
		mockMvc.perform(
				post("/boundarys/childLocationsByBoundaryId").param("boundaryId", "").param(string2, string7)
						.header(string9, string10).contentType(MediaType.APPLICATION_JSON_UTF8))
	...
	public void testShouldFetchAllBoundariesForBoundarytypenameAndHierarchytypename() throws Exception {
		...
		mockMvc.perform(post("/boundarys/boundariesByBndryTypeNameAndHierarchyTypeName").param(string2, string7)
				.param("boundaryTypeName", "Ward").param("hierarchyTypeName", "Admin")
				.header(string9, string10)).andExpect(status().isOk())
				.andExpect(content().contentType(MediaType.APPLICATION_JSON_UTF8))
				.andExpect(content().json(getFileContents(string8)));
	}
	...
	public void testShouldReturnBadRequestWhenBoundarytypenameAndHierarchytypenameIsEmpty() throws Exception {
		...
		mockMvc.perform(post("/boundarys/boundariesByBndryTypeNameAndHierarchyTypeName").param(string2, string7)
				.param("boundaryTypeName", "").param("hierarchyTypeName", "").header(string9, string10)
	...
	public void testShouldFetchAllBoundariesForBoundarytypeidAndtenantId() throws Exception {
		...
		when(boundaryService.getAllBoundariesByBoundaryTypeIdAndTenantId(any(Long.class), any(String.class)))
				.thenReturn(Collections.singletonList(expectedBoundary));
		mockMvc.perform(post("/boundarys/getByBoundaryType").param("boundaryTypeId", "7").param(string2, string2))
				.andExpect(status().isOk()).andExpect(content().contentType(MediaType.APPLICATION_JSON_UTF8))
				.andExpect(content().json(getFileContents(string8)));
	}
	...
	public void testShouldReturnBadRequestWhenBoundarytypeidAndTenmantIdIsEmpty() throws Exception {
		when(boundaryService.getAllBoundariesByBoundaryTypeIdAndTenantId(any(Long.class), any(String.class)))
				.thenReturn(null);
		mockMvc.perform(post("/boundarys/getByBoundaryType").param("boundaryTypeId", "").param(string2, "")
				.header(string9, string10).contentType(MediaType.APPLICATION_JSON_UTF8))
				.andExpect(status().isBadRequest());
	}
	...
	public void isShapeFileExist() throws Exception {
		...
		when(responseInfoFactory.createResponseInfoFromRequestInfo(any(RequestInfo.class), any(Boolean.class)))
				.thenReturn(responseInfo);
		mockMvc.perform(post("/boundarys/isshapefileexist").param(string2, string3)
	...
	public void testShouldReturnBadRequestWhenTenantIsNotThere() throws Exception {
		mockMvc.perform(post(string11).header(string9, string10)
				.contentType(MediaType.APPLICATION_JSON_UTF8)).andExpect(status().isBadRequest());
	}
	...
	public void testShouldReturnBoundaries() throws Exception {
		...
		mockMvc.perform(post(string11).param(string2, string3)
	...
	public void testShouldReturnBadRequestWhenTenantIsEmpty() throws Exception {
		mockMvc.perform(post(string11).param(string2, "").header(string9, string10)
				.contentType(MediaType.APPLICATION_JSON_UTF8)).andExpect(status().isBadRequest());
	}
    ...
    private List<Boundary> getSearchBoundaries(){
    	List<Boundary> boundaryList = new ArrayList<Boundary>();
    	Boundary boundary1 = Boundary.builder().id(1l).name("Srikakulam  Municipality").boundaryNum(1l).tenantId(string3).build();
    	BoundaryType boundaryType1 = BoundaryType.builder().id("1").name("City").hierarchy(1l).tenantId(string3).build();
    	boundary1.setBoundaryType(boundaryType1);
    	
    	Boundary boundary2 = Boundary.builder().id(2l).name("Zone-1").boundaryNum(1l).tenantId(string3).build();
    	BoundaryType boundaryType2 = BoundaryType.builder().id("3").name("Zone").hierarchy(3l).tenantId(string3).build();
