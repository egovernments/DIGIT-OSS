package org.egov.egovnationaldashboardingest.repository;
...
public class ElasticSearchRepository {
    ...
    public void indexFlattenedDataToES(String index, List<String> finalDocumentsToBeIndexed) {
        try {
            ...
            StringBuilder uri = new StringBuilder(applicationProperties.getElasticSearchHost() + BULK_ENDPOINT);
            ...
            Boolean errorWhileIndexingData = responseNode.get(BODY).get(ERRORS).asBoolean();
    ...
    public void findIfRecordAlreadyExists(StringBuilder uri) {
        try {
            ...
            String res = objectMapper.writeValueAsString(response);
            JsonNode responseNode = objectMapper.readValue(res, JsonNode.class);
            log.info(responseNode.get(BODY).toString());
            Integer recordsFound = responseNode.get(BODY).get(HITS).get(TOTAL).asInt();
            if (recordsFound > 0){
