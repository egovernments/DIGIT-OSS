...
public class ActionSearchRowMapper implements RowMapper<Action> {
	...
	public Action mapRow(final ResultSet rs, final int rowNum) throws SQLException {
		...
		if (!actionMap.containsKey(rs.getString(SERVICE_CODE))) {

			List<Action> actionList = new ArrayList<Action>();

			actionMap.put(rs.getString(SERVICE_CODE), prepareActionObject(rs, actionList));

		} else if (actionMap.containsKey(rs.getString(SERVICE_CODE))) {

			List<Action> actionList = actionMap.get(rs.getString(SERVICE_CODE));
