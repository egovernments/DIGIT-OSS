...
public class ActionRepository {
	...
	private String string1="queryparams";

	private String string2="enabled";

	private String string3="tenantid";

	private static final String STRING4=" SELECT s1.id,s1.code, s1.name, s1.parentmodule,s1.displayname,s1.enabled";
	...
	public List<Action> createAction(final ActionRequest actionRequest) {
		...
		for (Action action : actions) {
			batchValues.add(new MapSqlParameterSource("name", action.getName()).addValue("url", action.getUrl())
					.addValue("servicecode", action.getServiceCode()).addValue(string1, action.getQueryParams())
					.addValue("parentmodule", action.getParentModule()).addValue("ordernumber", action.getOrderNumber())
					.addValue("displayname", action.getDisplayName()).addValue(string2, action.isEnabled())
	...
	public boolean checkCombinationOfUrlAndqueryparamsExist(String url, String queryParams) {
		...
		parametersMap.put(string1, queryParams);
	...
	private Map<String, List<Action>> getActionsQueryBuilder(ActionRequest actionRequest) {
		...
		parametersMap.put(string3, actionRequest.getTenantId());
		...
		if (actionRequest.getEnabled() != null) {
			...
			parametersMap.put(string2, actionRequest.getEnabled());
		} else {
...
private List<Action> convertToAction(ActionRequest actionRequest,JSONArray actionsArray)
	...
	for (int i = 0; i < actionsArray.length(); i++) {
		...
		if(actionsArray.getJSONObject(i).has(string2)){
		act.setEnabled(actionsArray.getJSONObject(i).getBoolean(string2));
		...
		if(actionsArray.getJSONObject(i).has(string1)){
		act.setQueryParams(actionsArray.getJSONObject(i).get(string1).toString());
