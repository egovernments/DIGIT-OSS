...
class StateRepository {
    ...
    async insertNewState(userId, active, state) {
        const query = 'INSERT INTO eg_web_chat_state_v2 (user_id, active, state) VALUES ($1, $2, $3)';
        let result = await pool.query(query, [userId, active, state]);
        return result;
    }

    async updateState(userId, active, state) {
        const query = 'UPDATE eg_web_chat_state_v2 SET active = $2, state = $3 WHERE user_id = $1';
        let result = await pool.query(query, [userId, active, state]);
        return result;
    }
    ...
    async getActiveStateForUserId(userId) {
        ...
        const query = 'SELECT (state) FROM eg_web_chat_state_v2 WHERE user_id = $1 AND active = true';
    ...
    async getUserId(active){
        const query = 'SELECT DISTINCT user_id FROM eg_web_chat_state_v2 WHERE active = $1';
