...
class PGRService {
  ...
  async persistComplaint(user, slots, extraInfo) {
    const requestBody = JSON.parse(pgrCreateRequestBody);

    const { authToken } = user;
    const { userId } = user;
    const complaintType = slots.complaint;
    const { locality } = slots;
    const { city } = slots;
    ...
    if (slots.image) {
      const filestoreId = await this.getFileForFileStoreId(slots.image, city);
      const content = {
        documentType: 'PHOTO',
        filestoreId,
      };
    ...
    const url = config.egovServices.egovServicesHost + config.egovServices.pgrCreateEndpoint;
