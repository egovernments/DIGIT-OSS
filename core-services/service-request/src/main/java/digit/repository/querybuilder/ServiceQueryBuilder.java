...
public class ServiceQueryBuilder {
    ...
    public String getServiceSearchQuery(ServiceCriteria criteria, List<Object> preparedStmtList) {
        ...
        if (!ObjectUtils.isEmpty(criteria.getAttributes())) {
            if (!StringUtils.isEmpty(criteria.getAttributes().getAttributeCode())) {
                addClauseIfRequired(query, preparedStmtList);
                query.append(" attribute_value.attributeCode = ? ");
                preparedStmtList.add(criteria.getAttributes().getAttributeCode());
            }
            if (!StringUtils.isEmpty(criteria.getAttributes().getValue())) {
                addClauseIfRequired(query, preparedStmtList);
                query.append(" attribute_value.value ->> 'value' = ? ");
                preparedStmtList.add(criteria.getAttributes().getValue());
            }
        }
