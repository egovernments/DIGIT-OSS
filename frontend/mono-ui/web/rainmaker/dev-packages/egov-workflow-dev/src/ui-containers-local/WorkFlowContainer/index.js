...
class WorkFlowContainer extends React.Component {
    ...
    try {
      ...
      if (payload) {
        ...
        if(moduleName === "EDITRENEWAL"){
          window.location.href = `acknowledgement?${this.getPurposeString(label)}&applicationNumber=${applicationNumber}&licenseNumber=${licenseNumber}&FY=${nextFinancialYear}&tenantId=${tenant}&action=${moduleName}`;
        }
        if(moduleName === "DIRECTRENEWAL"){
          window.location.href = `acknowledgement?${this.getPurposeString(label)}&applicationNumber=${applicationNumber}&licenseNumber=${licenseNumber}&FY=${nextFinancialYear}&tenantId=${tenant}&action=${moduleName}`;
        }
      }
    } catch (e) {
  ...
  getRedirectUrl = (action, businessId, moduleName) => {
    ...
    else if(moduleName ==="EDITRENEWAL" || moduleName ==="DIRECTRENEWAL"){
      bservice = "TL";
      baseUrl =process.env.REACT_APP_NAME === "Citizen" ? "tradelicense-citizen" : "tradelicence";
    }
    else {
  ...
  getEmployeeRoles = (nextAction, currentAction, moduleName) => {
    ...
    if (nextAction === currentAction) {
      ...
      const states = find(data && data.states && data.states, { uuid: nextAction });
      states &&
        states.actions &&
  ...
  checkIfTerminatedState = (nextStateUUID, moduleName) => {
    ...
    const isLastState = data && data.states? find(data.states, { uuid: nextStateUUID }).isTerminateState : false;
    return isLastState;
  };
