...
const getAssessmentInfo = (propertyDetails, generalMDMSDataById) => {
  ...
  var construction_date =
    additionalDetails && additionalDetails.constructionYear ? moment(additionalDetails.constructionYear).format("DD-MM-YYYY") : null;
      ...
      let floor = [
        {
          ...
          value: unit.usageCategoryMinor
            ? "PROPERTYTAX_BILLING_SLAB_" + unit.usageCategoryMinor
            : unit.usageCategoryMajor
            ? "PROPERTYTAX_BILLING_SLAB_" + unit.usageCategoryMajor
            : "NA",
        },
        {
...
const AssessmentInfo = ({ properties, editIcon, generalMDMSDataById }) => {
  ...
  if (properties) {
    ...
    if (propertyDetails && propertyDetails.length > 0) {
      subUnitItems = getUnitInfo(
        propertyDetails[0]["units"],
        propertyDetails[0]["usageCategoryMajor"] ? propertyDetails[0]["usageCategoryMajor"] : "",
        propertyDetails[0]["buildUpArea"]
      );
  ...
  return (
    <PropertyInfoCard
      ...
      header={header}
      subSection={subUnitItems}
      hideSubsectionLabel={hideSubsectionLabel}
    ></PropertyInfoCard>
  );
};
