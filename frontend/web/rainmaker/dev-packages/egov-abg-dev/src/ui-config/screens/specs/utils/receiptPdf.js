...
const getSingleBillData = transformedData => {
  let singleBillData = {
    content: [
     ...
     {
       canvas: [
         { type: 'line', x1: -40, y1: 30, x2: 700, y2: 30, dash: {length: 10, space: 5} }
           ]
     },
     {
...
export const generateSingleBill = async rowData => {
  ...
  const data = allBills.find(
    item =>
      get(item, "consumerCode", "") === rowData[1]
  );
  if (isEmpty(data)) {
