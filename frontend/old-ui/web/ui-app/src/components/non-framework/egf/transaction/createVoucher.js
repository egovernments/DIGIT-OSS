...
class createVoucher extends Component {
  ...
  autoComHandler = (autoObject, path) => {
    ...
    Api.commonApiPost(url, query, {}, false, specifications[`egf.create`].useTimestamp).then(
      function(res) {
  ...
  addRow = (isAdd = false) => {
    ...
    if (isAdd) {
      if (
        !(
          currentData.vouchers[0].ledgers[currentData.vouchers[0].ledgers.length - 1].debitAmount ||
          currentData.vouchers[0].ledgers[currentData.vouchers[0].ledgers.length - 1].creditAmount
        )
      ) {
