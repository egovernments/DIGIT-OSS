...
const createProxy = createProxyMiddleware({
  ...
  target: process.env.REACT_APP_PROXY_API || "103.166.62.118:80",
  changeOrigin: true,
});
const assetsProxy = createProxyMiddleware({
  target: process.env.REACT_APP_PROXY_ASSETS || "103.166.62.118:80",
  changeOrigin: true,
});
...
const LicProxy = createProxyMiddleware({
  target: process.env.REACT_APP_PROXY_MDMS || "103.166.62.118:80",
  changeOrigin: true,
});
const docUploadProxy = createProxyMiddleware({
  target: process.env.REACT_APP_PROXY_UPLOAD_DOC || "103.166.62.118:80",
  changeOrigin: true,
});
const devRegistration = createProxyMiddleware({
  target: process.env.REACT_APP_PROXY_DEV_REG || "103.166.62.118:80",
  changeOrigin: true,
});
const LicNewRegistration = createProxyMiddleware({
  target: process.env.REACT_APP_PROXY_NEW_LIC || "103.166.62.118:80",
  changeOrigin: true,
});
const EgScrutinyProxy = createProxyMiddleware({
  target: process.env.REACT_APP_PROXY_SCRUTINY_EG || "103.166.62.118:80",
  changeOrigin: true,
});
