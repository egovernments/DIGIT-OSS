...
const createProxy = createProxyMiddleware({
  ...
  target: process.env.REACT_APP_PROXY_API || "http://tcp.ab.com:80",
  changeOrigin: true,
});
const assetsProxy = createProxyMiddleware({
  target: process.env.REACT_APP_PROXY_ASSETS || "http://tcp.ab.com:80",
  changeOrigin: true,
});
...
const LicProxy = createProxyMiddleware({
  target: process.env.REACT_APP_PROXY_MDMS || "http://tcp.ab.com:80",
  changeOrigin: true,
});
const docUploadProxy = createProxyMiddleware({
  target: process.env.REACT_APP_PROXY_UPLOAD_DOC || "http://tcp.ab.com:80",
  changeOrigin: true,
});
const devRegistration = createProxyMiddleware({
  target: process.env.REACT_APP_PROXY_DEV_REG || "http://tcp.ab.com:80",
  changeOrigin: true,
});
const EgScrutinyProxy = createProxyMiddleware({
  target: process.env.REACT_APP_PROXY_SCRUTINY_EG || "http://tcp.ab.com:80",
  changeOrigin: true,
});
