...
export const WorkflowService = {
  ...
  getDetailsById: async ({ tenantId, id, moduleCode, role, getTripData }) => {
    ...
    if (workflow && workflow.ProcessInstances) {
      ...
      const nextActions = location.pathname.includes("new-vehicle-entry") ? action_newVehicle : location.pathname.includes("dso") ? actionRolePair.filter((i)=> i.action !== "PAY") : tempCheckStatus.includes("WAITING_FOR_DISPOSAL") || tempCheckStatus.includes("PENDING_APPL_FEE_PAYMENT") ? actionRolePair : actionRolePair.filter((i)=> i.action !== "PAY");
