    ...
    const useBPADynamicData = () => {
        ...
        {select: (data) => {
            const obpsData = {
                ...
                dynamicDataTwo : data?.bpaTotalPlansScrutinized === 0 || data?.bpaTotalPlansScrutinized === null ? null : data?.bpaTotalPlansScrutinized + " " + t("BUILING_PLANS_SCRUTINISED_IN_LAST_12_MONTHS") + " " + t(tenantId),
            }
            return obpsData;
        }});
        return { isLoading, error, data, isSuccess };
