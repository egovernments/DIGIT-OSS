...
const DriverDetails = (props) => {
  ...
  const handleDeleteVendor = () => {
    let dsoDetails = driverData?.[0]?.dsoDetails;
    const formData = {
      vendor: {
        ...dsoDetails,
        status: "INACTIVE"
                        ...
                        <>
                          <div className={`${index === detail?.values?.length - 1 ? "row last" : "row"} border-none`}>
                            ...
                            <div className="value" style={{ color: '#f47738', display: 'flex' }}>
                              {t(value.value) || "N/A"}
                              {value.value === 'ES_FSM_REGISTRY_DETAILS_ADD_VENDOR' && <AddIcon className="" fill="#f47738" onClick={() => onActionSelect('ADD_VENDOR')} styles={{ cursor: 'pointer', marginLeft: '20px', height: '24px' }} />}
                              {value.value != 'ES_FSM_REGISTRY_DETAILS_ADD_VENDOR' && <EditIcon onClick={() => onActionSelect('EDIT_VENDOR')} style={{ cursor: 'pointer', marginLeft: '20px' }} />}
                              {value.value != 'ES_FSM_REGISTRY_DETAILS_ADD_VENDOR' && <DeleteIcon onClick={() => onActionSelect('DELETE_VENDOR')} className="delete" fill="#f47738" style={{ cursor: 'pointer', marginLeft: '20px' }} />}
                            </div>
                          </div>
                        </>
                      ) :
