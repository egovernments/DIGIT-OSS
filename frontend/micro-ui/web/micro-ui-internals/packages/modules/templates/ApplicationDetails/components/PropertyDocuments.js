          ...
          <div style={{ display: "flex", flexWrap: "wrap" }}>
            {document?.values?.map((value, index) => (
              <a target="_" href={pdfFiles[value.fileStoreId]?.split(",")[0]} style={{ minWidth: svgStyles?.minWidth ? svgStyles?.minWidth : "160px", marginRight: "20px" }} key={index}>
                ...
                {/* TODO, Later will move to classes */}
                <p style={checkLocation ? { marginTop: "8px", fontWeight: "bold", textAlign: "center", fontSize: "16px", lineHeight: "19px", color: "#505A5F" } : { marginTop: "8px", fontWeight: "bold", textAlign: "center" }}>{t(value?.title?value?.title:`DOC-${index+1}`)}</p>
              </a>
            ))}
          </div>
        </React.Fragment>
