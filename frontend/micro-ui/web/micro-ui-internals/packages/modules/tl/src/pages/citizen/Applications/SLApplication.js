...
const SLApplications = ({ view }) => {
  ...
  const getApplications = async () => {
    ...
    try {
      const Resp = await axios.post("/tl-services/SurrendOfLicenseRequest/_search", data);
      setLoader(false);
      setData(Resp?.data);
    } catch (error) {
      setLoader(false);
  ...
  return (
    <div>
      ...
      {/* <table className="customers" id="customers" style={{ borderCollapse: "collapse", width: "100%" }}>
        ...
        {data?.surrendOfLicense?.map((item, index) => {
          return (
            <tr key={`table${index}`}>
              ...
              <td
                ...
                onClick={() => {
                  ...
                  history.push({
                    pathname: "/digit-ui/citizen/obps/SurrenderLic",
