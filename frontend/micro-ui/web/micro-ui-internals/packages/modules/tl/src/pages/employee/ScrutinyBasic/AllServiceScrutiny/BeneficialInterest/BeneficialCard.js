...
import BeneficialBasic from "./BeneficialBasic";

const BeneficialCard = (props) => {
  const { id } = useParams();
  ...
  const [businessService, setBusinessService] = useState("CHANGE_OF_BENEFICIAL");
  ...
  const getScrutinyData = async () => {
    ...
    try {
      const Resp = await axios.post(`/tl-services/beneficial/_get?applicationNumber=${id}`, requestInfo).then((response) => {
        return response?.data;
      });
        console.log("Response From API1", Resp, Resp?.changeBeneficial);
      setScrutinyDetails(Resp?.changeBeneficial?.[0]);

      console.log("devDel1234", Resp?.changeBeneficial?.[0]);
      const loiNumber = Resp?.changeBeneficial?.[0]?.loiNumber;
      setApplicationData(Resp?.changeBeneficial?.[0]);
      setApplicationStatus(Resp?.changeBeneficial?.[0]?.status);
      setApplicationDetails({
        applicationData: Resp?.changeBeneficial?.[0],
        workflowCode: Resp?.changeBeneficial?.[0].businessService,
      });
          ...
          {showModal ? (
            <ActionModal
              ...
              applicationData={{
                ...
                workflowCode: applicationDetails?.applicationData?.workflowCode || "CHANGE_OF_BENEFICIAL",
              }}
