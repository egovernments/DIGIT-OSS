...
import CompletionLicBasic from "./CompletionLicBasic";

const CompletionLicCard = (props) => {
  const { id } = useParams();
  ...
  const [businessService, setBusinessService] = useState("COMPLETION_CERTIFICATE");
  ...
  const getScrutinyData = async () => {
    ...
    try {
      const Resp = await axios.post(`/tl-services/certicifate/_get?applicationNumber=${id}`, requestInfo).then((response) => {
        return response?.data;
      });
        console.log("Response From API1", Resp, Resp?.completionCertificate);
      setScrutinyDetails(Resp?.completionCertificate?.[0]);

      console.log("devDel1234", Resp?.completionCertificate?.[0]);
      const loiNumber = Resp?.completionCertificate?.[0]?.loiNumber;
      setApplicationData(Resp?.completionCertificate?.[0]);
      setApplicationStatus(Resp?.completionCertificate?.[0]?.status);
      setApplicationDetails({
        applicationData: Resp?.completionCertificate?.[0],
        workflowCode: Resp?.completionCertificate?.[0].businessService,
      });
  ...
  const submitAction = async (data = {}, nocData = false, isOBPS = {}) => {
    let tempdata = data || {};
    tempdata.completionCertificate[0].additionalDetails = additionalDetails;
    ...
    try {
      ...
      const response = await axios.post("/tl-services/certicifate/_update", body);
          ...
          {showModal ? (
            <ActionModal
              ...
              applicationData={{
                ...
                workflowCode: applicationDetails?.applicationData?.workflowCode || "COMPLETION_CERTIFICATE",
              }}
...
export default CompletionLicCard;
