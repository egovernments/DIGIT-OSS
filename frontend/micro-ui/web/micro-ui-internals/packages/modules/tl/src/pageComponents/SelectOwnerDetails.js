  ...
  return (
    ...
    <FormStep config={config} onSelect={goNext} onSkip={onSkip} t={t} isDisabled={false} forcedError={t(error)}>
      {formState.map((field, index) => {
        return (
          <div key={`${field}-${index}`}>
            <div style={typeOfOwner === "MULTIOWNER" ? {border:"solid",borderRadius:"5px",padding:"10px",paddingTop:"20px",marginTop:"10px",borderColor:"#f3f3f3",background:"#FAFAFA"}:{}}>
            <CardLabel style={typeOfOwner === "MULTIOWNER" ?{marginBottom:"-15px"}: {}}>{`${t("TL_NEW_OWNER_DETAILS_NAME_LABEL")}`}</CardLabel>
           {typeOfOwner === "MULTIOWNER" && <LinkButton
            label={<div >
              <span>
