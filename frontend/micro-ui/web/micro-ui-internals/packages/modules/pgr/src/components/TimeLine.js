        ...
        return (
          <PendingForAssignment
            ...
            customChild={getCommentsInCustomChildComponent({
              comment,
              ...(isFirstPendingForAssignment ? { auditDetails } : { thumbnailsToShow, auditDetails }),
            })}
          />
        );
    ...
    <React.Fragment>
      ...
      {timeline && totalTimelineLength > 0 ? (
        <ConnectingCheckPoints>
          {timeline.map(
            ({ status, caption, auditDetails, timeLineActions, performedAction, wfComment: comment, thumbnailsToShow, assigner }, index, array) => {
