        ...
        const inboxColumns = () => [
    {
      ...
      Cell: ({ row }) => {
        return (
          <div>
            <span className="link">
              <Link to={`/digit-ui/employee/obps/${row.original["businessService"] === 'SERVICE_PLAN' ? 'ServiceScrutiny' : row.original["businessService"] === 'ELECTRICAL_PLAN' ? 'ElectricalScrutiny' : row.original["businessService"] === "BG_NEW" ? 'ScrutinyForm' : row.original["businessService"] === "TECHNICAL_PROFESSIONAL"
