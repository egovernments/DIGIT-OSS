...
const GetActionMessage = (props) => {
  ...
  if (props.isSuccess) {
    return t("CS_WATER_DISCONNECTION_APPLICATION_SUCCESS");
  } else if (props.isLoading) {
    return t("CS_WATER_DISCONNECTION_APPLICATION_PENDING");
  } else if (!props.isSuccess) {
    return t("CS_WATER_DISCONNECTION_APPLICATION_FAILED");
  }
};
...
const WSAcknowledgement = ({ data, onSuccess, clearParams, userType }) => {
  const { t } = useTranslation();
  ...
  return (
    ...
    // data?.serviceName?.code === "WATER"? WSmutation.isLoading || WSmutation.isIdle : (data?.serviceName?.code === "SEWERAGE") ? SWmutation.isLoading || SWmutation.isIdle : (WSmutation.isLoading || WSmutation.isIdle || SWmutation.isLoading || SWmutation.isIdle)) ? (<Loader />) : (
    <Card>
      ...
      {((data?.serviceName?.code === "WATER"? WSmutation.isSuccess : SWmutation.isSuccess) && userType!== "employee") &&<SubmitBar label={t("WS_DOWNLOAD_ACK_FORM")} onSubmit={handleDownloadPdf} />}       
      {(SWmutation.isSuccess && userType== "employee") && (
        <LinkButton
          label={
        ...
        />)}
      <Link className={`${userType== "employee" && 'action-bar-wrap'}`} to={`/digit-ui/${userType}`}>
        {userType == "employee" ? <SubmitBar label={t("CORE_COMMON_GO_TO_HOME")} onSubmit={handleDownloadPdf} /> : 
