...
const WSDisconnectionDocumentsEmployee = ({ t, config, onSelect, userType, formData, setError: setFormError, clearErrors: clearFormErrors, formState }) => {
  const tenantId = Digit.ULBService.getCurrentTenantId();
  const stateId = Digit.ULBService.getStateId();
  const [documents, setDocuments] = useState(formData?.DocumentsRequired?.documents || []);
  const [error, setError] = useState(null);
  const wsDocsData = 'DisconnectionDocuments';
  let action = "create";
...
export default WSDisconnectionDocumentsEmployee;
