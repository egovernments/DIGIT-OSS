...
const styles = {
  buttonStyle: { display: "flex", justifyContent: 'flex-end', color: 'rgb(68,214,44)' },
  headerStyle: {
    marginTop: '10px',
    fontSize: '16px',
    fontWeight: '700',
...
const ArrearSummary = ({ bill = {} }) => {
  ...
  let fees = {};
  let sortedBillDetails = bill?.billDetails
  ...
  return (
    <React.Fragment>
      ...
      {!showArrear && <div style={styles.buttonStyle}>
        <button type="button" onClick={() => {
        setShowArrear(true)
      }}>{t("CS_SHOW_CARD")}</button></div>}
      
      {showArrear && <div style={styles.buttonStyle}>
        <button type="button" onClick={() => {
        setShowArrear(false)
      }}>{t("CS_HIDE_CARD")}</button></div>}
