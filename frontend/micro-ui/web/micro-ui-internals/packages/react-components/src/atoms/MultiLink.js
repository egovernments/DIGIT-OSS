...
const MultiLink = ({ className, onHeadClick, displayOptions = false, options, label, icon, showOptions, downloadBtnClassName, optionsClassName }) => {
  ...
  return (
    <div className={className} ref={menuRef}>
      <div className={`multilink-labelWrap ${downloadBtnClassName}`} onClick={onHeadClick}>
        {icon ? icon : <PrimaryDownlaodIcon />}
        <LinkButton label={label || t("CS_COMMON_DOWNLOAD")} className="multilink-link-button" />
      </div>
      {displayOptions ? (
        <div className={`multilink-optionWrap ${optionsClassName}`}>
