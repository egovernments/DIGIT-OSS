...
const FilterFormField = ({children, className}) => {
	return <div className={`filter-form-field ${className}`}>
		{children}
	</div>
}

const FilterForm = ({clearAll, onMobileExclusiveFilterPopupFormClose = () => null , showMobileFilterFormPopup = false, children}) => {
	...
	<div className="filter-card">
	  <div className="heading" style={{ alignItems: "center" }}>
		...
		<span className="clear-search desktop-only" onClick={clearAll} style={{ border: "1px solid #e0e0e0", padding: "6px" }}>
		...
		{showMobileFilterFormPopup ? <span onClick={onMobileExclusiveFilterPopupFormClose} className="mobile-only">
			<CloseSvg />
		</span> : null}
	  </div>
		{children}
		<SubmitBar className="w-fullwidth" label={t("ES_COMMON_SEARCH")} submit />
	</div>
  </div>
}
