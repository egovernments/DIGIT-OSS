...
const getTradeDetails = (application, t) => {
  return {
    ...
    values: [
      ...
      { title: t("reports.tl.fromDate"), value: application?.validFrom ? window.Digit.DateUtils.ConvertTimestampToDate(application?.validFrom, "dd/MM/yyyy") : t("CS_NA") },
      { title: t("reports.tl.toDate"), value: application?.validTo ? window.Digit.DateUtils.ConvertTimestampToDate(application?.validTo, "dd/MM/yyyy") : t("CS_NA") },
      ...
      { title: t("TL_NEW_TRADE_DETAILS_TRADE_COMM_DATE_LABEL"), value: window.Digit.DateUtils.ConvertTimestampToDate(application?.commencementDate, "dd/MM/yyyy") || t("CS_NA"), },
...
const getPTAcknowledgementData = async (application, tenantInfo, t) => {
  ...
  let res;
  if (filesArray) {
    res = await window.Digit.UploadServices.Filefetch(filesArray, window.Digit.ULBService.getStateId());
  }
  return {
    t: t,
    ...
    details: [
      ...
      //       value: window.Digit.DateUtils.ConvertTimestampToDate(application?.applicationDate, "dd/MM/yyyy"),
      //     },
      //   ],
      // },
