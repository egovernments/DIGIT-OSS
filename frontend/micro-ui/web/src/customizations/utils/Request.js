...
const requestInfo = () => ({
  authToken: window.Digit.UserService.getUser().access_token,
});

const authHeaders = () => ({
  "auth-token": window.Digit.UserService.getUser().access_token,
});

const userServiceData = () => ({ userInfo: window.Digit.UserService.getUser().info });
...
export const Request = async ({
  ...
  if (method.toUpperCase() === "POST") {
    ...
    if (locale) {
      data.RequestInfo = { ...data.RequestInfo, msgId: `string|${window.Digit.StoreData.getCurrentLanguage()}` };
    }
  }
