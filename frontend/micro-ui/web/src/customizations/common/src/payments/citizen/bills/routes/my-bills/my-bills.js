...
export const BillList = ({ billsList, currentPath, businessService }) => {
  ...
  // const { mobileNumber } = window.Digit.UserService.getUser()?.info;

  const searchResult = window.Digit.Hooks.useApplicationsForBusinessServiceSearch({ filters: { consumerCodes: consumerCodes.join() }, businessService });
  ...
  */
  ...
  const [getKeyNotesConfig, setConfig] = useState(() => window.Digit.ComponentRegistryService?.getComponent("getBillDetailsConfigWithBusinessService"));
