...
export const SuccessfulPayment = (props) => {
  ...
  const { eg_pg_txnid: egId, workflow: workflw } = window.Digit.Hooks.useQueryParams();
  ...
  const { isLoading, data, isError } = window.Digit.Hooks.usePaymentUpdate({ egId }, business_service, {
    retry: false,
    staleTime: Infinity,
    refetchOnWindowFocus: false,
  });

  const { label } = window.Digit.Hooks.useApplicationsForBusinessServiceSearch({ businessService: business_service }, { enabled: false });

  // const { data: demand } = window.Digit.Hooks.useDemandSearch(
  //   { consumerCode, businessService: business_service },
  //   { enabled: !isLoading, retry: false, staleTime: Infinity, refetchOnWindowFocus: false }
  // );

  // const { data: billData, isLoading: isBillDataLoading } = window.Digit.Hooks.useFetchPayment(
  ...
  const { data: reciept_data, isLoading: recieptDataLoading } = window.Digit.Hooks.useRecieptSearch(
    {
      tenantId,
  ...
  const { data: generatePdfKey } = window.Digit.Hooks.useCommonMDMS(tenantId, "common-masters", "ReceiptKey", {
    select: (data) =>
  ...
  const isMobile = window.window.Digit.Utils.browser.isMobile();
  ...
  const printCertificate = async () => {
    ...
    const applicationDetails = await window.Digit.TLService.search({ applicationNumber: consumerCode, tenantId });
    const generatePdfKeyForTL = "tlcertificate"

    if (applicationDetails) {
      let response = await window.Digit.PaymentService.generatePdf(state, { Licenses: applicationDetails?.Licenses }, generatePdfKeyForTL);
      const fileStore = await window.Digit.PaymentService.printReciept(state, { fileStoreIds: response.filestoreIds[0] });
  ...
  const printReciept = async () => {
    ...
    if (!paymentData?.fileStoreId) {
      response = await window.Digit.PaymentService.generatePdf(state, { Payments: [payments.Payments[0]] }, generatePdfKey);
    }
    const fileStore = await window.Digit.PaymentService.printReciept(state, { fileStoreIds: response.filestoreIds[0] });
    ...
    if (taxPeriodFrom && taxPeriodTo) {
      ...
      if (workflw === "mcollect") {
        from =
          ...
          " " +
          window.Digit.Utils.date.monthNames[new Date(fromPeriod).getMonth() + 1].toString() +
          " " +
          new Date(fromPeriod).getFullYear().toString();
        to =
          new Date(toPeriod).getDate() +
          " " +
          window.Digit.Utils.date.monthNames[new Date(toPeriod).getMonth() + 1] +
          " " +
