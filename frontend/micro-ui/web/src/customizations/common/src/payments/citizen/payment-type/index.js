...
export const SelectPaymentType = (props) => {
  ...
  const { tenantId: __tenantId, authorization } = window.Digit.Hooks.useQueryParams();
  ...
  const tenantId = state?.tenantId || __tenantId || window.Digit.ULBService.getCurrentTenantId();
  const stateTenant = tenantId.split(".")[0];
  const { control, handleSubmit } = useForm();
  const { data: menu, isLoading } = window.Digit.Hooks.useCommonMDMS(stateTenant, "DIGIT-UI", "PaymentGateway");
  const { data: paymentdetails, isLoading: paymentLoading } = window.Digit.Hooks.useFetchPayment({ tenantId: tenantId, consumerCode, businessService }, {});
  ...
  const onSubmit = async (d) => {
    ...
    try {
      const data = await window.Digit.PaymentService.createCitizenReciept(tenantId, filterData);
