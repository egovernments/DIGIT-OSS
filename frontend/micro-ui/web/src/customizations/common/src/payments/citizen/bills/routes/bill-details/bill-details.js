...
const BillDetails = ({ paymentRules, businessService }) => {
  ...
  const { workflow: wrkflow, tenantId: _tenantId } = window.Digit.Hooks.useQueryParams();
  const [bill, setBill] = useState(state?.bill);
  const tenantId = state?.tenantId || _tenantId || window.Digit.UserService.getUser().info?.tenantId;
  const { data, isLoading } = state?.bill ? { isLoading: false } : window.Digit.Hooks.useFetchPayment({ tenantId, businessService, consumerCode });
  ...
  const { key, label } = window.Digit.Hooks.useApplicationsForBusinessServiceSearch({ businessService }, { enabled: false });
