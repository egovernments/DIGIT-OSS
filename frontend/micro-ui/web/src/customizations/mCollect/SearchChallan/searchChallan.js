...
const SearchChallan = ({ config: propsConfig, formData }) => {
  ...
  console.log("prasad Servicecateogry", Servicecateogry);
  const allCities = window.Digit.Hooks.mcollect.usemcollectTenants()?.sort((a, b) => a?.i18nKey?.localeCompare?.(b?.i18nKey));
  const [mobileNumberError, setmobileNumberError] = useState(null);
  // moduleCode, type, config = {}, payload = []

  const { data: Menu, isLoading } = window.Digit.Hooks.mcollect.useMCollectMDMS(tenantId, "BillingService", "BusinessService");
  if (isLoading) {
    return <Loader />;
  }
