...
const ChallanSearchResults = ({ template, header, actionButtonLabel }) => {
  ...
  const { mobileNumber, challanNo, Servicecategory,tenantId } = window.Digit.Hooks.useQueryParams();
  const filters = {};
  ...
  const result = window.Digit.Hooks.mcollect.useMcollectSearchBill({ tenantId, filters });
