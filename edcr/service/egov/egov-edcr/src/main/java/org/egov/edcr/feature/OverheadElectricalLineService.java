...
public class OverheadElectricalLineService extends FeatureProcess {
    ...
    public Plan validate(Plan pl) {
        ...
        for (ElectricLine electricalLine : pl.getElectricLine()) {
            if (Boolean.TRUE.equals(electricalLine.getPresentInDxf())) {
    ...
    public Plan process(Plan pl) {
        ...
        for (ElectricLine electricalLine : pl.getElectricLine()) {
                if (Boolean.TRUE.equals(electricalLine.getPresentInDxf()) 
                		&& electricalLine.getVoltage() != null
	...
	/*
	 * private boolean processVerticalDistance(ElectricLine electricalLine,
	 * PlanDetail planDetail, String remarks1, String remarks2) {
	 * 
	 * boolean verticalDistancePassed = false;
	 * 
	 * if (electricalLine.getVerticalDistance() != null) { String actualResult =
	 * electricalLine.getVerticalDistance().toString() + DcrConstants.IN_METER;
	 * String expectedResult = "";
	 * 
	 * if (electricalLine.getVoltage().compareTo(BigDecimal.valueOf(VOLTAGE_11000))
	 * < 0) {
	 * 
	 * expectedResult = VERTICAL_DISTANCE_11000.toString() + DcrConstants.IN_METER;
	 * if (electricalLine.getVerticalDistance().compareTo(VERTICAL_DISTANCE_11000)
	 * >= 0) verticalDistancePassed = true;
	 * 
	 * } else if
	 * (electricalLine.getVoltage().compareTo(BigDecimal.valueOf(VOLTAGE_11000)) >=
	 * 0 && electricalLine.getVoltage().compareTo(BigDecimal.valueOf(VOLTAGE_33000))
	 * <= 0) {
	 * 
	 * expectedResult = VERTICAL_DISTANCE_33000.toString() + DcrConstants.IN_METER;
	 * if (electricalLine.getVerticalDistance().compareTo(VERTICAL_DISTANCE_33000)
	 * >= 0) verticalDistancePassed = true;
	 * 
	 * } else if
	 * (electricalLine.getVoltage().compareTo(BigDecimal.valueOf(VOLTAGE_33000)) >
	 * 0) {
	 * 
	 * Double totalVertficalOHE = VERTICAL_DISTANCE_33000.doubleValue() + 0.3 *
	 * Math.ceil(
	 * electricalLine.getVoltage().subtract(BigDecimal.valueOf(VOLTAGE_33000))
	 * .divide(BigDecimal.valueOf(VOLTAGE_33000), 2, RoundingMode.HALF_UP)
	 * .doubleValue()); expectedResult = totalVertficalOHE + DcrConstants.IN_METER;
	 * if (electricalLine.getVerticalDistance()
	 * .compareTo(BigDecimal.valueOf(totalVertficalOHE)) >= 0) {
	 * verticalDistancePassed = true; } } if (verticalDistancePassed) {
	 * setReportOutputDetails(planDetail, SUB_RULE_31,
	 ...
	 * remarks1,electricalLine.getVoltage().toString() + DcrConstants.IN_KV); } else
	 * { setReportOutputDetails(planDetail, SUB_RULE_31,
	 * DcrConstants.VERTICAL_ELECTRICLINE_DISTANCE + electricalLine.getNumber(),
	 * expectedResult, actualResult, Result.Not_Accepted.getResultVal(),
	 * remarks2,electricalLine.getVoltage().toString() + DcrConstants.IN_KV); }
	 * 
	 * } return verticalDistancePassed; }
	 */
