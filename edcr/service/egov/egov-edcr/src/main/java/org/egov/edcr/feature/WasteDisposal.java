...
public class WasteDisposal extends FeatureProcess {
	...
	public Plan validate(Plan pl) {
		 ...
		 * errors.put(WASTEDISPOSAL, edcrMessageSource.getMessage(OBJECTNOTDEFINED, new
		 * String[] { WASTEDISPOSAL }, LocaleContextHolder.getLocale()));
		 * pl.addErrors(errors); } } }
		 */
		return pl;
	}
	...
	public Plan process(Plan pl) {
		 ...
		 * scrutinyDetail.setKey("Common_Waste Disposal"); if
		 * (pl.getUtility().getLiquidWasteTreatementPlant().isEmpty()) { if
		 * (!pl.getUtility().getWasteDisposalUnits().isEmpty()) {
		 * setReportOutputDetailsWithoutOccupancy(pl, SUB_RULE_26A,
		 * SUB_RULE_26A_DESCRIPTION, "", OBJECTDEFINED_DESC,
		 * Result.Accepted.getResultVal()); if (pl.getUtility().getWells().isEmpty()) {
		 * for (org.egov.common.entity.edcr.WasteDisposal wasteDisposal :
		 * pl.getUtility().getWasteDisposalUnits()) { if
		 * (wasteDisposal.getType().equals(DcrConstants.PROPOSED)) {
		 * printOutputForProposedWasteDisposal(pl); } } } } else {
		 * setReportOutputDetailsWithoutOccupancy(pl, SUB_RULE_26A,
		 * SUB_RULE_26A_DESCRIPTION, "", OBJECTNOTDEFINED_DESC,
	 ...
	 * 
	 * private void printOutputForProposedWasteDisposal(Plan pl) { String subRule;
	 * String subRuleDesc; boolean valid = false; for (RoadOutput roadOutput :
	 * pl.getUtility().getWellDistance()) {
	 * 
	 * BigDecimal minimumDistance; if
	 * (checkConditionForLeachPitToBoundary(roadOutput)) { subRule =
	 * SUB_RULE_104_4_WD; subRuleDesc = SUB_RULE_104_4_PLOT_DESCRIPTION_WD;
	 * minimumDistance = BigDecimal.valueOf(1.2); } else continue;
	 * printReportOutput(pl, subRule, subRuleDesc, valid, roadOutput,
	 * minimumDistance);
	 * 
	 * } }
	 * 
	 * private void printReportOutput(Plan pl, String subRule, String subRuleDesc,
	 * boolean valid, RoadOutput roadOutput, BigDecimal minimumDistance) {
	 * HashMap<String, String> errors = new HashMap<>(); if
	 * (minimumDistance.compareTo(BigDecimal.ZERO) == 0) {
	 ...
	 * pl.addErrors(errors); } else { if (roadOutput.distance != null &&
	 * roadOutput.distance.compareTo(BigDecimal.ZERO) > 0 &&
	 * roadOutput.distance.compareTo(minimumDistance) >= 0) valid = true; if (valid)
	 * { setReportOutputDetailsWithoutOccupancy(pl, subRule, subRuleDesc,
	 * minimumDistance.toString() + IN_METER, roadOutput.distance + IN_METER,
	 * Result.Accepted.getResultVal()); } else {
	 * setReportOutputDetailsWithoutOccupancy(pl, subRule, subRuleDesc,
	 * minimumDistance.toString() + IN_METER, roadOutput.distance + IN_METER,
	 * Result.Not_Accepted.getResultVal()); } }
	 * 
	 ...
	 * return Objects.equals(Integer.valueOf(roadOutput.colourCode),
	 * COLOUR_CODE_LEACHPIT_TO_PLOT_BNDRY); }
	 */
