...
public class WaterTreatmentPlant extends FeatureProcess {
	...
	public Plan validate(Plan pl) {
		 ...
		 * pl.getUtility() != null) { // liquid waste treatment plant defined or not if
		 ...
		 * > 0 && pl.getUtility().getLiquidWasteTreatementPlant().isEmpty()) {
		 * errors.put(SUB_RULE_53_5_DESCRIPTION,
		 * edcrMessageSource.getMessage(OBJECTNOTDEFINED, new String[] {
		 * SUB_RULE_53_5_DESCRIPTION }, LocaleContextHolder.getLocale()));
	...
	public Plan process(Plan pl) {
		 ...
		 * scrutinyDetail.setKey("Common_Water Treatment Plant"); if
		 * (pl.getVirtualBuilding() != null &&
		 * !pl.getVirtualBuilding().getOccupancies().isEmpty()) { for (OccupancyType
		 * occupancyType : pl.getVirtualBuilding().getOccupancies()) { if
		 * (checkOccupancyTypeEqualsToNonConditionalOccupancyTypes(occupancyType)) {
		 ...
		 * pl.getVirtualBuilding().getTotalBuitUpArea() != null &&
		 * pl.getVirtualBuilding().getTotalBuitUpArea().compareTo(TWOTHOUSANDFIVEHUNDER)
		 * > 0) { processLiquidWasteTreatment(pl); break; } } }
		 */
		return pl;
	}
	...
	private void processLiquidWasteTreatment(Plan pl) {
		if (!pl.getUtility().getLiquidWasteTreatementPlant().isEmpty()) {
			setReportOutputDetailsWithoutOccupancy(pl, SUB_RULE_53_5, SUB_RULE_53_5_DESCRIPTION, "", OBJECTDEFINED_DESC,
