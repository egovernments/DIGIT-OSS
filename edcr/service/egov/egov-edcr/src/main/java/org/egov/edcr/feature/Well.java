...
public class Well extends FeatureProcess {
	...
	// private static final String WELL_DISTANCE_FROM_ROAD = "Minimum distance from
	// well to road";
	...
	public Plan validate(Plan pl) {
		/*
		 * HashMap<String, String> errors = new HashMap<>(); if (pl != null &&
		 * pl.getUtility() != null) { if (!pl.getUtility().getWells().isEmpty()) { if
		 ...
		 * pl.getUtility().getWasteDisposalUnits().get(0).getType().equals(DcrConstants.
		 * PROPOSED)) { if (pl.getUtility().getWellDistance().stream()
		 ...
		 * pl.addErrors(errors); } if (pl.getUtility().getWellDistance().stream()
		 ...
		 * pl.addErrors(errors); } } else if
		 * (pl.getUtility().getWells().get(0).getType() != null &&
		 * pl.getUtility().getWells().get(0).getType().equals(DcrConstants.PROPOSED) &&
		 * pl.getUtility().getWasteDisposalUnits().get(0).getType().equals(DcrConstants.
		 * EXISTING)) { if (pl.getUtility().getWellDistance().stream()
		 * .noneMatch(roadOutput -> roadOutput.colourCode.equals(String.valueOf(7)))) {
		 * errors.put(SUB_RULE_104_2_DESCRIPTION,
		 * edcrMessageSource.getMessage(OBJECTNOTDEFINED, new String[] {
		 * SUB_RULE_104_2_DESCRIPTION }, LocaleContextHolder.getLocale()));
		 ...
		 * (pl.getUtility().getWells().get(0).getType() != null &&
		 * pl.getUtility().getWells().get(0).getType().equals(DcrConstants.PROPOSED)) {
		 * if (pl.getUtility().getWellDistance().stream() .noneMatch(roadOutput ->
		 * roadOutput.colourCode.equals(String.valueOf(7)))) {
		 * errors.put(SUB_RULE_104_2_DESCRIPTION,
		 * edcrMessageSource.getMessage(OBJECTNOTDEFINED, new String[] {
		 * SUB_RULE_104_2_DESCRIPTION }, LocaleContextHolder.getLocale()));
		 * pl.addErrors(errors); } if (pl.getUtility().getWellDistance().stream()
	...
	public Plan process(Plan pl) {
		 ...
		 * scrutinyDetail.setKey("Common_Well"); if
		 * (!pl.getUtility().getWells().isEmpty()) { if
		 * (!pl.getUtility().getWasteDisposalUnits().isEmpty()) { if
		 * (pl.getUtility().getWells().get(0).getType() != null &&
		 * pl.getUtility().getWasteDisposalUnits().get(0).getType() != null) { if
		 * (pl.getUtility().getWells().get(0).getType() != null &&
		 * pl.getUtility().getWells().get(0).getType().equals(DcrConstants.PROPOSED) &&
		 * pl.getUtility().getWasteDisposalUnits().get(0).getType().equals(DcrConstants.
		 * PROPOSED)) { printOutputForProposedWellAndProposedWasteDisposal(pl); } else
		 * if (pl.getUtility().getWells().get(0).getType() != null &&
		 * pl.getUtility().getWells().get(0).getType().equals(DcrConstants.PROPOSED) &&
		 * pl.getUtility().getWasteDisposalUnits().get(0).getType().equals(DcrConstants.
		 * EXISTING)) { printOutputForProposedWellAndExistingWasteDisposal(pl); } else
		 * if (pl.getUtility().getWells().get(0).getType() != null &&
		 * pl.getUtility().getWells().get(0).getType().equals(DcrConstants.EXISTING) &&
		 * pl.getUtility().getWasteDisposalUnits().get(0).getType().equals(DcrConstants.
		 * PROPOSED)) { printOutputForExistingWellAndProposedWasteDisposal(pl); } } }
	...
	private void printReportOutput(Plan pl, String subRule, String subRuleDesc, boolean valid, RoadOutput roadOutput,
		...
		if (minimumDistance.compareTo(BigDecimal.ZERO) == 0) {
			errors.put(WELL_DISTANCE_FROMBOUNDARY, getLocaleMessage(WELL_ERROR_COLOUR_CODE_DISTANCE_FROMROAD,
