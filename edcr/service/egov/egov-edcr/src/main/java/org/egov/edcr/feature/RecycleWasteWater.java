...
public class RecycleWasteWater extends FeatureProcess {
	// private static final BigDecimal ONETHOUSANDFIVEHUNDER =
	...
	// private static final BigDecimal TWOTHOUSANDFIVEHUNDER =
	...
	public Plan validate(Plan pl) {
		 ...
		 * pl.getUtility() != null) { // waste water recycle plant defined or not if
		 ...
		 * pl.getVirtualBuilding().getTotalBuitUpArea().compareTo(ONETHOUSANDFIVEHUNDER)
		 * > 0 &&
		 * pl.getVirtualBuilding().getTotalBuitUpArea().compareTo(TWOTHOUSANDFIVEHUNDER)
		 * < 0 && pl.getUtility().getWasteWaterRecyclePlant().isEmpty()) {
		 * errors.put(SUB_RULE_53_6_DESCRIPTION,
		 * edcrMessageSource.getMessage(OBJECTNOTDEFINED, new String[] {
		 * SUB_RULE_53_6_DESCRIPTION }, LocaleContextHolder.getLocale())); break; } } }
		 * }
		 */
		return pl;
	}
	...
	public Plan process(Plan pl) {
		 ...
		 * scrutinyDetail.setKey("Common_Waste Water Recycle Plant"); if
		 * (pl.getVirtualBuilding() != null &&
		 * !pl.getVirtualBuilding().getOccupancies().isEmpty()) { for (OccupancyType
		 * occupancyType : pl.getVirtualBuilding().getOccupancies()) { if
		 * (checkOccupancyTypeEqualsToNonConditionalOccupancyTypes(occupancyType)) {
		 ...
		 * > 0 &&
		 * pl.getVirtualBuilding().getTotalBuitUpArea().compareTo(TWOTHOUSANDFIVEHUNDER)
		 * < 0) { processWasteWaterRecyclePlant(pl); break; } } }
		 */
	...
	private void processWasteWaterRecyclePlant(Plan pl) {
		if (!pl.getUtility().getWasteWaterRecyclePlant().isEmpty()) {
			setReportOutputDetailsWithoutOccupancy(pl, SUB_RULE_53_6, SUB_RULE_53_6_DESCRIPTION, "", OBJECTDEFINED_DESC,
	...
	private boolean checkOccupancyTypeEqualsToNonConditionalOccupancyTypes(OccupancyType occupancyType) {
		return occupancyType.equals(OccupancyType.OCCUPANCY_B1) || occupancyType.equals(OccupancyType.OCCUPANCY_B2)
