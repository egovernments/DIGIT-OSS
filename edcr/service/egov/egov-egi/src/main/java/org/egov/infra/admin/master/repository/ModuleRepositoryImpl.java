...
public class ModuleRepositoryImpl implements ModuleRepositoryCustom {
    ...
    public List<Object[]> fetchModulesByParentModuleId(final Long parentId, final Long userId) {
        ...
        final StringBuilder sql = new StringBuilder().
                ...
                append("AND action.enabled = true AND action.id = roleaction.actionid  AND roleaction.roleid IN (SELECT roleid FROM state.eg_userrole userrole ").
                ...
                append("(select roleid from state.eg_userrole ur where ur.userid = :userId))) order by typeflag desc,ordernumber asc");
