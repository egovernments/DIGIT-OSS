...
def connect():
    ...
    property_detailsquery = pd.read_sql_query("select eg_pt_property.*, adr.locality as locality from eg_pt_property left outer join eg_pt_address adr on eg_pt_property.id = adr.propertyid where eg_pt_property.status='ACTIVE' and eg_pt_property.createdtime < 1606780799000 AND eg_pt_property.tenantId != 'pb.testing'", conn)
