...
public class GeneralLedgerHibernateDAO  implements GeneralLedgerDAO {
	...
	public String getActualsPrev(final String accCode, final String functionId, final String budgetingType) throws ApplicationException {
		...
		/*
		 * Budgeting type is hardcoded here to frame the query. 1 - sum(debitamount) -
		 * sum(creditamount) 2 - sum(creditamount) 3 - sum(debitamount) Based on the
		 * budgeting type the query will differ.
		 */
		if (!functionId.equalsIgnoreCase("0")) {
			if (budgetingType.equalsIgnoreCase("1")) {
				hqlQuery.append("select sum(cgeneralledger.debitAmount)-sum(cgeneralledger.creditAmount)")
						.append(" from CGeneralLedger cgeneralledger,CVoucherHeader cvoucherheader")
						.append(" where cgeneralledger.voucherHeaderId.id=cvoucherheader.id")
						.append(" and cvoucherheader.fiscalPeriodId in (:fiscalperiodId)")
						.append(" and cgeneralledger.functionId=:functionId")
						.append(" and cgeneralledger.glcode like :accCode");
				params.put("fiscalperiodId", fiscalperiodId);
				params.put("functionId", functionId);
				params.put("accCode", accCode + "|| %");
			} else if (budgetingType.equalsIgnoreCase("2")) {
				hqlQuery.append("select sum(cgeneralledger.creditAmount) from CGeneralLedger cgeneralledger,").append(
						"CVoucherHeader cvoucherheader where cgeneralledger.id.voucherHeaderId=cvoucherheader.id ")
						.append("and cvoucherheader.fiscalPeriodId in (:fiscalperiodId)")
						.append(" and cgeneralledger.functionId=:functionId")
						.append(" and cgeneralledger.glcode like :accCode");
				params.put("fiscalperiodId", fiscalperiodId);
				...
				hqlQuery.append("select sum(cgeneralledger.debitAmount) from CGeneralLedger cgeneralledger,").append(
						"CVoucherHeader cvoucherheader where cgeneralledger.id.voucherHeaderId=cvoucherheader.id")
						.append(" and cvoucherheader.fiscalPeriodId in (:fiscalPeriodId)")
						.append(" and cgeneralledger.functionId=:functionId")
						.append(" and cgeneralledger.glcode like :accCode");
			...
			if (budgetingType.equalsIgnoreCase("1")) {
				...
				hqlQuery.append("select sum(cgeneralledger.creditAmount) from CGeneralLedger cgeneralledger,").append(
						"CVoucherHeader cvoucherheader where cgeneralledger.voucherHeaderId.id=cvoucherheader.id")
						.append(" and cvoucherheader.fiscalPeriodId in (:fiscalPeriodId)")
						.append(" and cgeneralledger.glcode like :accCode");
				params.put("fiscalPeriodId", fiscalperiodId);
				params.put("accCode", accCode + "|| %");
	...
	public String getActualsDecCurr(final String accCode, final String functionId, final String budgetingType) throws ApplicationException{
		...
		calendar.set(Calendar.YEAR, Integer.parseInt(temp[0]));
		/*
		 * Here we have hardcoded the month(december) and day(31). Calendar month starts
		 * from 0 and hence december will be 11.
		 */
		...
		if (!functionId.equalsIgnoreCase("0")) {
			if (budgetingType.equalsIgnoreCase("1")) {
				hqlQuery.append("select sum(cgeneralledger.debitAmount)-sum(cgeneralledger.creditAmount)")
						.append(" from CGeneralLedger cgeneralledger,CVoucherHeader cvoucherheader")
						.append(" where cgeneralledger.voucherHeaderId.id=cvoucherheader.id and cvoucherheader.voucherDate >=:startdate")
						.append(" and cvoucherheader.voucherDate <=:endDate")
						.append(" and cgeneralledger.functionId=:functionId")
						.append(" and cgeneralledger.glcode like :accCode");
				...
				hqlQuery.append("select sum(cgeneralledger.creditAmount) from CGeneralLedger cgeneralledger,").append(
						"CVoucherHeader cvoucherheader where cgeneralledger.voucherHeaderId.id=cvoucherheader.id")
						.append(" and cvoucherheader.voucherDate >=:startdate")
						.append(" and cvoucherheader.voucherDate <=:endDate")
						.append(" and cgeneralledger.functionId=:functionId")
						.append("  and cgeneralledger.glcode like :accCode");
				params.put("startdate", startdate);
			...
			if (budgetingType.equalsIgnoreCase("1")) {
				...
				hqlQuery.append("select sum(cgeneralledger.creditAmount) from CGeneralLedger cgeneralledger,").append(
						"CVoucherHeader cvoucherheader where cgeneralledger.voucherHeaderId.id=cvoucherheader.id")
						.append(" and cvoucherheader.voucherDate >=:startdate and cvoucherheader.voucherDate <=:endDate")
						.append(" and cgeneralledger.glcode like :accCode");
				params.put("startdate", startdate);
				params.put("endDate", endDate);
				params.put("accCode", accCode + "|| %");
	...
	public BigDecimal getGlAmountForBudgetingType(final Long budType, final List glcodeList, final String finYearID,
		...
		try {
			...
			if (budType == 1) {
				qryStr.append("select abs(sum(cgeneralledger.debitAmount)-sum(cgeneralledger.creditAmount))")
						.append(" from CGeneralLedger cgeneralledger,CVoucherHeader cvoucherheader ");
			} else if (budType == 2) {
				qryStr.append("select abs(sum(cgeneralledger.creditAmount)) from CGeneralLedger cgeneralledger")
						.append(",CVoucherHeader cvoucherheader ");
			} else if (budType == 3) {
				qryStr.append("select abs(sum(cgeneralledger.debitAmount)) from CGeneralLedger cgeneralledger,")
						.append("CVoucherHeader cvoucherheader ");
			}
			...
			cond = new StringBuilder(
					" where cgeneralledger.voucherHeaderId.id=cvoucherheader.id and cvoucherheader.fiscalPeriodId in")
							.append(" ( select cfiscalperiod.id from CFiscalPeriod cfiscalperiod")
							.append(" where cfiscalperiod.financialYearId =:finYearID ) and cgeneralledger.glcode in ( :glcodeList) ")
							.toString();
