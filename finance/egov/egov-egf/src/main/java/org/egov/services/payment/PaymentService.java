...
public class PaymentService extends PersistenceService<Paymentheader, Long> {
	...
	public Paymentheader transitionWorkFlow(final Paymentheader paymentheader, WorkflowBean workflowBean) {
		...
		if (FinancialConstants.BUTTONREJECT.equalsIgnoreCase(workflowBean.getWorkFlowAction())) {
			...
			paymentheader.transition().end().withSenderName(user.getName())
					...
					.withOwner(getPosition()).withNextAction(wfmatrix.getNextAction());
	...
	public List<ChequeAssignment> getPaymentVoucherForRTGSInstrument(final Map<String, String[]> parameters,
		 ...
		 * (EgBillSubType) persistenceService.find(
		 * " from EgBillSubType where name = ?",
		 * FinancialConstants.BILLSUBTYPE_TNEBBILL); if (egSubType.getId() !=
		 * null) sql.append(
		 * " and (bmis.billsubtype is null or bmis.billsubtype not in (" +
		 * egSubType.getId() + "))");
	...
	/*
	 * private Map getSubledgerAmtForDeduction(final Long payVhId) { if
	 * (LOGGER.isDebugEnabled()) LOGGER.debug(
	 * "Starting getSubledgerAmtForDeduction..."); final Map<String, BigDecimal>
	 * map = new HashMap<String, BigDecimal>(); final Query query = getSession()
	 * .createSQLQuery(
	 ...
	 * + " and misbill.payvhid =" + payVhId +
	 * " group by gld.detailtypeid,gld.detailkeyid ");
	 * query.setParameterList("glcodeIdList", cBillGlcodeIdList); final
	 * List<Object[]> list = query.list(); if (list != null && !list.isEmpty())
	 * for (final Object[] ob : list) map.put(ob[0].toString() + DELIMETER +
	 * ob[1].toString(), (BigDecimal) ob[2]); if (LOGGER.isDebugEnabled())
	 * LOGGER.debug("Completed getSubledgerAmtForDeduction."); return map; }
	 */
