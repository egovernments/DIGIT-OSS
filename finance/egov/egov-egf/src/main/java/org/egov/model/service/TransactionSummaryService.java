...
public class TransactionSummaryService {
	...
	public List<TransactionSummary> searchTransactionsForNonSubledger(Long finYear, Long fund, Long functn,
			String department, Long glcodeId) {
		TypedQuery<TransactionSummary> query = entityManager.createQuery(
				new StringBuilder("select ts from TransactionSummary ts where ts.financialyear.id=:finYear")
						.append(" and ts.fund.id=:fund and ts.functionid.id=:functn and ts.departmentCode=:department")
						.append(" and ts.glcodeid.id=:glcodeId and ts.glcodeid.id not in")
	...
	public List<TransactionSummary> searchTransactionsForSubledger(Long finYear, Long fund, Long functn,
			String department, Long glcodeId, Integer accountDetailTypeId, Integer accountDetailKeyId) {
		TypedQuery<TransactionSummary> query = entityManager.createQuery(
				new StringBuilder("select ts from TransactionSummary ts where ts.financialyear.id=:finYear")
						.append(" and ts.fund.id=:fund and ts.functionid.id=:functn and ts.departmentCode=:department")
						.append(" and ts.glcodeid.id=:glcodeId and ts.accountdetailkey=:accountDetailKeyId")
    ...
    public TransactionSummary getTransactionSummary(Long glcodeId, Long accountDetailTypeId, Integer accountDetailKey) {
		TypedQuery<TransactionSummary> query = entityManager
				.createQuery(new StringBuilder("from TransactionSummary where glcodeid.id=:glcodeid")
						.append(" and accountdetailtype.id=:accountdetailtype and accountdetailkey=:accountdetailkey")
						.toString(), TransactionSummary.class);
        query.setParameter("glcodeid", glcodeId);
