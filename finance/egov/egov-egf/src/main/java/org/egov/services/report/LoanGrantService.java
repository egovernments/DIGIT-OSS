...
public class LoanGrantService extends PersistenceService {
	...
	public List<Object> schemeUtilizationBy(final Integer schemeId, final Integer subSchemeId, final Date fromDate,
		...
		if (subSchemeId != null) {
			schemeUtilSql.append("select ss.name as subScheme, pc.code as code, vh.voucherNumber as vouchernumber,")
					.append(" vh.voucherDate as voucherdate, gld.amount as amount, gld.detailkeyid as id ")
					.append("from voucherheader vh, generalledger gl, generalledgerdetail gld , egf_subscheme_project ssp,")
					.append(" egw_projectcode pc,sub_scheme ss ")
					.append("	where  vh.id= gl.voucherheaderid  and vh.status not in (1,2,4) ")
					.append(" and gl.id= gld.generalledgerid ");
			if (fromDate != null) {
			...
			schemeUtilSql.append(" and gld.detailtypeid = :detailTypeId and gld.detailkeyid= ssp.projectcodeid ")
					...
					.append(" and pc.id= gld.detailkeyid ")
					.append(" and pc.id= ssp.projectcodeid and vh.fundid=:fundId ");
			...
			if (projectCodeIdList != null && projectCodeIdList.size() > 0) {
				schemeUtilSql.append(" and ssp.projectcodeid in :projectCodeId ");
				params.put("projectCodeId", projectCodeIdList);
			}
			...
			schemeUtilSql.append("select ss.name as subScheme, pc.code as code, vh.voucherNumber as vouchernumber,")
					.append(" vh.voucherDate as voucherdate, gld.amount as amount, gld.detailkeyid as id ")
					.append("from voucherheader vh, generalledger gl, generalledgerdetail gld , egf_subscheme_project ssp,")
					.append(" egw_projectcode pc,sub_scheme ss,scheme s ")
					.append("	where vh.id= gl.voucherheaderid  and vh.status not in (1,2,4) ")
					.append(" and gl.id= gld.generalledgerid ");
			if (fromDate != null) {
				schemeUtilSql.append(" and vh.voucherdate>=:voucherFromDate");
			...
			schemeUtilSql.append(" and gld.detailtypeid=:detailTypeId and gld.detailkeyid= ssp.projectcodeid ")
					.append(" and ssp.subschemeid=ss.id and ss.schemeid=s.id and s.id = :schemeId")
					.append(" and ss.id=ssp.subschemeid and pc.id= gld.detailkeyid and pc.id= ssp.projectcodeid")
	...
	public List<LoanGrantBean> fundingPatternBy(final Integer subSchemeId, final Integer schemeId) {
		...
		sql.append(" select ss.name as subScheme, fa.name  as name ,  sum(lgd.percentage) as amount ")
				.append(" from egf_LoanGrantDetail lgd, egf_LoanGrantHeader lgh,egf_fundingAgency fa,sub_scheme ss ");
		...
		if (schemeId != null && subSchemeId == null)
			...
			patternSql.setParameter("subSchemeId", subSchemeId);
    ...
    public List<Object> searchGC(final Integer schemeId, final Integer subSchemeId, final Date fromDate, final Date toDate,
        ...
        if (agencyId != null & agencyId != -1) {
			...
			if (subSchemeId != null)
				...
				fundingAgencyList = findAllBy(new StringBuilder(
						"select distinct lgd.fundingAgency.id from LoanGrantDetail lgd ,LoanGrantHeader lg,")
								.append("SubScheme ss,Scheme s where lg.subScheme.id=ss.id and s.id=ss.scheme.id")
								.append(" and lg.id=lgd.header.id and s.id=?").toString(),
						schemeId);
	...
	private List<Object> getDataByAgency(final Integer schemeId, final Integer subSchemeId, final Date fromDate,
		...
		final StringBuilder sql = new StringBuilder();
		final Map<String, Object> params = new HashMap<>();
		sql.append(" select * from (select distinct vh.vouchernumber as voucherNumber, gld1.amount as amount,").append(
				" null as agencyAmount,pc.code as code, gld1.detailkeyid as detailKey, gld1.detailtypeid detailType ")
				.append("from voucherheader vh, generalledger gl1,generalledger gl2, generalledgerdetail gld1,")
				.append(" generalledgerdetail gld2, ").append("egf_subscheme_project ssp,egw_projectcode pc ");
		if (subSchemeId == null)
		...
		sql.append(" and  gld2.detailkeyid = :agencyId").append(" union ").append(
				" select distinct vh.vouchernumber as voucherNumber,null as amount,gld2.amount as agencyAmount,")
				.append("null as code, gld2.detailKeyid as detailKey, gld2.detailtypeid detailType ")
				.append("from voucherheader vh, generalledger gl1,generalledger gl2, generalledgerdetail gld1,")
				.append(" generalledgerdetail gld2 ")
				.append(",egf_loangrantdetail lgd,egf_loanGrantHeader lg,egf_subscheme_project ssp ");
		...
		sql.append(" where vh.id= gl1.voucherheaderid and gl1.id= gld1.generalledgerid ")
				.append("and gl2.id=gld2.generalledgerid and gl2.voucherheaderid=vh.id and gl1.creditamount>0 ")
				.append("and gl2.debitamount>0 and gld1.detailtypeid=:pcTypeId")
				.append(" and gld2.detailtypeid=:faTypeId and vh.type='Journal Voucher' and vh.name='JVGeneral' ")
				.append("and lg.id=lgd.headerid and ssp.projectcodeid=gld1.detailkeyid and vh.fundid=:fundId ");
		params.put("pcTypeId", pcTypeId);
		params.put("faTypeId", faTypeId);
		params.put("fundId", fundId);
		...
		params.put("agencyId", agencyId);
		if (subSchemeId != null) {
			sql.append(" and lgd.agencyId = :lgdAgencyId");
	...
	private List<LoanGrantBean> getGrantAmountBy(final Integer schemeId, final Integer subSchemeId,
		...
		final StringBuilder gaSql = new StringBuilder();
		gaSql.append("select fa.name as agencyName, sum( case when lgd.grantamount = null THEN 0")
				.append(" else lgd.grantamount end)*100000 as grantAmount")
				.append(" from egf_loangrantheader lg, egf_loangrantdetail lgd, egf_fundingagency fa");
		if (subSchemeId == null)
		...
		gaSql.append(" and lgd.agencyid=:agencyId").append(" group by fa.name");
		if (LOGGER.isInfoEnabled())
		...
		if (subSchemeId == null)
			...
			gaSQLQuery.setParameter("subSchemeId", subSchemeId);
		gaSQLQuery.setParameter("agencyId", agencyId);
	...
	public List<Object> getLoanBy(final Integer schemeId, final Long agencyId, final Integer faTypeId,
		...
		if (agencyId != null && agencyId != -1)
			...
			ss.append("select distinct lgd.fundingAgency.id from LoanGrantDetail lgd ,LoanGrantHeader lg,SubScheme ss,")
					.append("Scheme s where lg.subScheme.id=ss.id and s.id=ss.scheme.id and lg.id=lgd.header.id and s.id=?");
	...
	public List<Object> getLoanByAgency(final Integer schemeId, final Long agencyId, final Integer faTypeId,
		...
		sql.append("SELECT DISTINCT vh.vouchernumber AS voucherNumber,  gld.amount    AS amount,  ")
				...
				.append(" AND gl.id             = gld.generalledgerid  AND gl.debitamount    >0  ")
				.append(" AND gld.detailtypeid  =  :faTypeId AND vh.type            ='Payment'  ")
				.append(" AND vh.name            ='Direct Bank Payment' and vh.status in (0,5)   ")
				.append(" and vmis.schemeid=:schemeId and vh.fundid=:fundId and vmis.voucherheaderid=vh.id");
		if (agencyId != null && agencyId != -1)
		...
		if (agencyId != null && agencyId != -1)
			loanSql.setParameter("agencyId", agencyId);
	...
	private List<LoanGrantBean> getLoanAmountBy(final Integer schemeId, final Long agencyId) {
		...
		loanSql.append("select fa.name as agencyName, sum( case when lgd.loanamount  = null then 0")
				.append(" else lgd.loanamount)*100000 as loanAmount")
				.append(" from egf_loangrantheader lg, egf_loangrantdetail lgd, egf_fundingagency fa,sub_scheme ss,scheme s")
				.append(" where lg.id= lgd.headerid and lgd.agencyid=fa.id ").append(" and lgd.agencyid=:agencyId")
				.append(" and lg.subSchemeId=ss.id").append(" and s.id=:schemeId").append(" and s.id=ss.schemeid ")
