...
public class BudgetDefinitionService {
	...
	private String generateMaterializedPath(final Budget budget) {
		return budget.getParent() == null ? String.valueOf(budgetDefinitionRepository.getRootBudgetsCount() + 1)
				: budget.getParent().getMaterializedPath().concat(".").concat(
						String.valueOf(budgetDefinitionRepository.getChildBudgetsCount(budget.getParent()) + 1));
	}
	...
	public List<Budget> search(final BudgetDefinitionSearchRequest budgetDefinitionSearchRequest) {
		if (budgetDefinitionSearchRequest.getFinancialYearId() != null
				&& budgetDefinitionSearchRequest.getSearchBere() != null)
			return budgetDefinitionRepository.findByIsbereIsAndFinancialYearIdIsOrderByFinancialYearIdAscNameAsc(
					budgetDefinitionSearchRequest.getSearchBere(), budgetDefinitionSearchRequest.getFinancialYearId());
		if (budgetDefinitionSearchRequest.getFinancialYearId() != null
				&& budgetDefinitionSearchRequest.getSearchBere() == null)
			return budgetDefinitionRepository.findByFinancialYearIdIsOrderByFinancialYearIdAscNameAsc(
					budgetDefinitionSearchRequest.getFinancialYearId());
		if (budgetDefinitionSearchRequest.getFinancialYearId() == null
				&& budgetDefinitionSearchRequest.getSearchBere() != null)
			return budgetDefinitionRepository
					.findByIsbereIsOrderByFinancialYearIdAscNameAsc(budgetDefinitionSearchRequest.getSearchBere());
		else
	...
	public List<Budget> getParentList(final String isbere, final Long financialYearId, final List<Long> budgetIdList) {
		if (budgetIdList.isEmpty())
			return budgetDefinitionRepository.findByIsActiveBudgetTrueAndIsbereIsAndFinancialYearIdIs(isbere,
					financialYearId);
		else
	...
	public EgwStatus getBudgetApprovedStatus() {
		return egwStatusHibernate.getStatusByModuleAndCode(FinancialConstants.BUDGET,
