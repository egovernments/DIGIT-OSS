...
public class FundFlowService extends PersistenceService {
	...
	public List<FundFlowBean> getOutStandingPayments(final Date asPerDate, final Long fundId) {
		...
		final StringBuilder outstandingPaymentQryStr = new StringBuilder();
		...
		if (condtitionalAppConfigIsPresent) {
			...
			outstandingPaymentQryStr.append("SELECT DISTINCT( ba.accountnumber)      AS accountNumber , ")
					.append(" ROUND(SUM(ph.paymentamount)/100000,2) AS outStandingBPV")
					...
					.append(" where ph.state_id =state.id and empinfo.pos_id= state.owner and empinfo.functionary_id=func.id ")
					.append(" and empinfo.isactive=true ")
					.append(" and empinfo.DESIGNATIONID=desg.DESIGNATIONID and vh.id =ph.voucherheaderid")
					.append(" and  ba.id=ph.bankaccountnumberid").append(" and desg.DESIGNATION_NAME like :designation")
					.append(" and func.NAME like :functionaryName ");

			queryParams.put("designation", designationName);
			queryParams.put("functionaryName", functionaryName);
			...
			outstandingPaymentQryStr.append(" and vh.voucherdate <=:voucherDate")
					.append("  and vh.voucherdate >=:finStartDate").append(" group by accountNumber  ");

			queryParams.put("voucherDate", voucherDate);
			queryParams.put("finStartDate", START_FINANCIALYEAR_DATE);
		} else {
			outstandingPaymentQryStr.append("SELECT DISTINCT( ba.accountnumber)      AS accountNumber , ")
					.append(" ROUND(SUM(ph.paymentamount)/100000,2) AS outStandingBPV")
					.append(" FROM voucherheader vh,paymentheader ph,bankaccount ba,eg_wf_states state")
					.append(" where ph.state_id =state.id and vh.id =ph.voucherheaderid and  ba.id=ph.bankaccountnumberid");
			if (fundId != null && fundId != -1) {
				outstandingPaymentQryStr.append(" and vh.fundid = :fundId");
				outstandingPaymentQryStr.append(" and ba.fundid = :fundId");
				queryParams.put("fundid", fundId);
			...
			outstandingPaymentQryStr.append(" and vh.voucherdate <= :voucherDate")
					.append("  and vh.voucherdate >= :finStartDate").append(" and state.value like ")
					.append(stateWithoutCondition).append(" group by accountNumber  ");
		...
		final Query outstandingQry = getSession().createSQLQuery(outstandingPaymentQryStr.toString())
				.addScalar("accountNumber").addScalar("outStandingBPV")
				.setResultTransformer(Transformers.aliasToBean(FundFlowBean.class));
		queryParams.entrySet().forEach(entry -> outstandingQry.setParameter(entry.getKey(), entry.getValue()));
	...
	public List<FundFlowBean> getConcurrancePayments(final Date asPerDate, final Long fundId) {
		...
		// get BPVs for the cuurent date which are in the workflow
		final StringBuilder conCurrancePaymentQryStr = new StringBuilder();
		final Map<String, Object> queryParams = new HashMap<>();

		conCurrancePaymentQryStr.append("SELECT DISTINCT( ba.accountnumber)      AS accountNumber , ")
				.append(" ROUND(SUM(ph.paymentamount)/100000,2) AS concurranceBPV")
				.append(" FROM voucherheader vh,paymentheader ph,bankaccount ba,eg_wf_states state where ph.state_id =state.id ")
				.append("	and vh.id =ph.voucherheaderid and  ba.id=ph.bankaccountnumberid and  vh.voucherdate >= :finStartDate");
		...
		conCurrancePaymentQryStr.append(" and to_char(created_date,'dd-Mon-yyyy') =:voucherDate")
				.append(" and ( state.value in (:voucherStatus) OR vh.status=0 ) group by accountNumber  ");

		queryParams.put("voucherDate", voucherDate);
		queryParams.put("voucherStatus", voucherStatus);
		...
		final Query conCurranceQry = getSession().createSQLQuery(conCurrancePaymentQryStr.toString())
				.addScalar("accountNumber").addScalar("concurranceBPV")
				.setResultTransformer(Transformers.aliasToBean(FundFlowBean.class));
		queryParams.entrySet().forEach(entry -> conCurranceQry.setParameter(entry.getKey(), entry.getValue()));
	...
	public List<FundFlowBean> getAllpaymentAccounts(final Long fundId) {
		final StringBuilder allPaymentAccounts = new StringBuilder();
		allPaymentAccounts
				.append("select ba.id as bankAccountId, ba.accountnumber as accountNumber, coa.glcode as glcode,")
				.append("b.code as bankName ,fd.name as fundName ")
				.append("from Chartofaccounts  coa,  fund fd, bankaccount ba left outer join bankbranch bb on ba.branchid=bb.id")
				.append(" left outer join bank b on bb.bankid=b.id where  coa.id=ba.glcodeid and ba.fundid= fd.id")
				.append(" and ba.isactive=true and ba.type in ('PAYMENTS','RECEIPTS_PAYMENTS') ");
		if (fundId != null && fundId != -1)
		...
		final Query allPaymentAccountsQry = getSession().createSQLQuery(allPaymentAccounts.toString())
				.addScalar("bankAccountId").addScalar("accountNumber").addScalar("glcode").addScalar("bankName")
				.addScalar("fundName").setResultTransformer(Transformers.aliasToBean(FundFlowBean.class));
		if (fundId != null && fundId != -1)
			allPaymentAccountsQry.setParameter("fundid", fundId);
	...
	public List<FundFlowBean> getAllReceiptAccounts(final Long fundId) {
		...
		allAccounts.append("select ba.id as bankAccountId, ba.accountnumber as accountNumber, coa.glcode as glcode,")
				.append("b.code as bankName ,fd.name as fundName, ")
				.append(" case when ba.narration = null then 0 else (")
				.append(String.format("case when instr(ba.narration,%s",
						FinancialConstants.BANKACCOUNT_WALKIN_PAYMENT_DESCRIPTION))
				.append(",1)  = 1 then 1 else 0 end ) end as walkinPaymentAccount ")
				.append(" from Chartofaccounts  coa, fund fd, bankaccount ba left outer join bankbranch bb")
				.append(" on ba.branchid=bb.id left outer ")
				.append(" join bank b on bb.bankid=b.id where coa.id=ba.glcodeid and ba.fundid= fd.id")
				.append(" and ba.isactive=true and ba.type in ('RECEIPTS') ");
		if (fundId != null && fundId != -1)
		...
		final Query allAccountsQry = getSession().createSQLQuery(allAccounts.toString()).addScalar("bankAccountId")
				...
				.setResultTransformer(Transformers.aliasToBean(FundFlowBean.class));
		if (fundId != null && fundId != -1)
			allAccountsQry.setParameter("fundid", fundId);
	...
	public List<FundFlowBean> getContraReceiptsForTheDay(final Date asPerDate, final Long fundId) {
		...
		temp.append(" SELECT gl.glcodeid as codeId, ba.accountnumber as accountNumber, b.name as bankName,").append(
				"round(SUM(case when gl.debitamount = NULL then 0 else gl.debitamount end)/100000,2) AS btbReceipt FROM")
				.append(" contrajournalvoucher CV  , voucherheader vh ,  generalledger gl, bankaccount ba, bankbranch bb,bank b WHERE ");
		if (fundId != null && fundId != -1)
			temp.append(" vh.fundid =:fundId AND ba.fundid =:fundId and");
		temp.append(" vh.voucherdate :voucherDate and gl.voucherheaderid= vh.id  and vh.name in (:vhName) ").append(
		...
		final Query tempQry = getSession().createSQLQuery(temp.toString()).addScalar("accountNumber")
				.addScalar("bankName").addScalar("btbReceipt")
				.setResultTransformer(Transformers.aliasToBean(FundFlowBean.class));
		if (fundId != null && fundId != -1)
			tempQry.setParameter("fundId", fundId);
		tempQry.setParameter("voucherDate", voucherDate);
		tempQry.setParameterList("vhName", Arrays.asList(FinancialConstants.CONTRAVOUCHER_NAME_BTOB,
	...
	/**
	 * get Receipt Bank Accounts of selected Fund which has Contra payment for
	 * current day
	 * 
	 * @param voucherDate
	 * @return
	...
	public List<FundFlowBean> getContraPaymentsForTheDay(final Date asPerDate, final Long fundId) {
		...
		qry.append(" SELECT gl.glcodeid as codeId, ba.accountnumber as accountNumber, b.name as bankName, ").append(
				"round(SUM(case when gl.creditamount = NULL then 0 else gl.creditamount end)/100000,2) AS btbPayment FROM")
				.append(" contrajournalvoucher CV  , voucherheader vh ,  generalledger gl, bankaccount ba, bankbranch bb,bank b WHERE ");
		if (fundId != null && fundId != -1)
		...
		qry.append("vh.voucherdate = :voucherDate and gl.voucherheaderid = vh.id and vh.name in (:vhName) ")
				.append("  and cv.voucherheaderid=vh.id and ba.id= cv.frombankaccountid and ba.glcodeid= gl.glcodeid ")
				.append("AND vh.status =0  and ba.fundid=vh.fundid  ")
				.append("  and bb.bankid= b.id and ba.branchid=bb.id GROUP BY gl.glcodeId,ba.accountnumber,b.name ");
		List<FundFlowBean> tempList;
		final Query q = getSession().createSQLQuery(qry.toString()).addScalar("accountNumber").addScalar("bankName")
				.addScalar("btbPayment").setResultTransformer(Transformers.aliasToBean(FundFlowBean.class));
		if (fundId != null && fundId != -1)
			q.setParameter("fundId", fundId);
		q.setParameter("voucherDate", voucherDate);
		q.setParameterList("vhName", Arrays.asList(FinancialConstants.CONTRAVOUCHER_NAME_BTOB,
				FinancialConstants.CONTRAVOUCHER_NAME_INTERFUND));
		tempList = q.list();
		if (LOGGER.isDebugEnabled())
	...
	/**
	 * get Receipt Bank Accounts of selected Fund which has Contra payment for
	 * current day When you use contraJournal voucher put fund condition for
	 * voucherheader which will remove duplicate entry duplicate is coming since
	 * Interfund transfer creates two vouchers with two different funds.
	 * 
	 * @param voucherDate
	 * @return
	 */
	...
	private BigDecimal getContraPayment(final Long bankaccountId, final Date asPerDate, final Long accountGlcodeId) {
		...
		if (accountGlcodeId != null) {
			qry.append(
					" select case when SUM(case when gl.creditamount = NULL then 0 else gl.creditamount end) = null then 0")
					.append(" else SUM(case when gl.creditamount = NULL then 0 else gl.creditamount end) end as payment from ")
					.append("  voucherheader vh, generalledger gl where vh.id=gl.voucherheaderid and gl.glcodeId=:glcode")
					.append("  and vh.name in (:vhName) ")
					.append("  and vh.fiscalperiodid in (select id from fiscalperiod where financialyearid=(select f.id")
					.append(" from financialyear f where CURRENT_DATE between f.startingdate and f.endingdate))")
					.append(" and vh.voucherdate=:voucherdate and vh.status =0");
			params.put("glcode", accountGlcodeId);
			params.put("vhName", Arrays.asList(FinancialConstants.CONTRAVOUCHER_NAME_BTOB,
					FinancialConstants.CONTRAVOUCHER_NAME_INTERFUND));
			params.put("voucherdate", sqlformat.format(asPerDate));
		} else {
			qry.append(
					" select case when SUM(case when gl.creditamount = NULL then 0 else gl.creditamount end) = null then 0")
					.append(" else SUM(case when gl.creditamount = NULL then 0 else gl.creditamount end) end as payment")
					.append(" from BankAccount acc,")
					.append(" voucherheader vh, generalledger gl where vh.id=gl.voucherheaderid and acc.glcodeId= gl.glcodeId ")
		...
		final Query query = getSession().createSQLQuery(qry.toString());
		...
		if (LOGGER.isDebugEnabled())
			LOGGER.debug("Contra Payments For BankId " + accountGlcodeId + " And Date " + sqlformat.format(asPerDate)
					+ " is : " + contraPayment);
	...
	private BigDecimal getContraReceipt(final Long bankaccountId, final Date asPerDate, final Long accountGlcodeId) {
		...
		if (accountGlcodeId != null) {
			qry.append(
					" select case when SUM(case when gl.debitamount = NULL then 0 else gl.debitamount end) = null then")
					.append(" 0 else SUM(case when gl.debitamount = NULL then 0 else gl.debitamount end) end as receipt from ")
					.append(" voucherheader vh, generalledger gl where vh.id=gl.voucherheaderid and gl.glcodeId=:glcode")
					.append("  and vh.name in (:vhName) ")
					.append("  and vh.fiscalperiodid in (select id from fiscalperiod where financialyearid=(select f.id")
					.append(" from financialyear f where CURRENT_DATE between f.startingdate and f.endingdate))")
			...
			qry.append(
					" select case when SUM(case when gl.debitamount = NULL then 0 else gl.debitamount end) = null then")
					.append(" 0 else SUM(case when gl.debitamount = NULL then 0 else gl.debitamount end) end as receipt")
					.append(" from BankAccount acc,")
					.append(" voucherheader vh, generalledger gl where vh.id=gl.voucherheaderid and acc.glcodeid= gl.glcodeid ")
					.append("  and vh.name in (:vhName) and vh.voucherdate=:voucherdate and acc.id=:bankaccountId and vh.status =0");
			params.put("vhName", Arrays.asList(FinancialConstants.CONTRAVOUCHER_NAME_BTOB,
		...
		if (LOGGER.isDebugEnabled())
			LOGGER.debug("Contra Receipt For BankId " + accountGlcodeId + " And Date " + sqlformat.format(asPerDate)
					+ " is : " + contraReceipt);
		return contraReceipt;
	}

	/**
	 * it is for single bankaccount Will return the concurrence done for the day for
	 * the give bank account
	 * 
	 * @param bankaccountId
	 * @param asPerDate
	 * @return
	...
	private BigDecimal getOutStandingPayment(final Long bankaccountId, final Date asPerDate) {
		...
		final StringBuilder outstandingPaymentQryStr = new StringBuilder();
		outstandingPaymentQryStr.append(
				"SELECT case when SUM(case when ph.paymentamount = null then 0 else ph.paymentamount end ) = null then 0")
				.append(" else SUM(case when ph.paymentamount = null then 0 else ph.paymentamount) end AS concurranceBPV")
				.append(" FROM voucherheader vh right join  paymentheader ph on vh.id=ph.voucherheaderid,bankaccount ba,")
				.append("eg_wf_states state where ph.state_id     =state.id ")
				.append("	and vh.id =ph.voucherheaderid and  ba.id=ph.bankaccountnumberid and ba.id = :bankaccountId")
				.append(" and vh.fiscalperiodid in (select id from fiscalperiod where financialyearid=(select f.id ")
				.append("from financialyear f where CURRENT_DATE between f.startingdate and f.endingdate))")
				.append(" and vh.voucherdate >= :startFinDate and to_char(created_date,'dd-Mon-yyyy') = :asPerDate")
				.append(" and ( state.value in (:voucherStatus) OR vh.status=0 ) group by accountNumber  ");
		...
		final List list = getSession().createSQLQuery(outstandingPaymentQryStr.toString())
				.setParameter("bankaccountId", bankaccountId).setParameter("startFinDate", START_FINANCIALYEAR_DATE)
				.setParameter("asPerDate", sqlformat.format(asPerDate)).setParameter("voucherStatus", voucherStatus)
				.list();
		if (!list.isEmpty())
