...
@ParentPackage("egov")
@Results({ @Result(name = BaseFormAction.NEW, location = "onlineReceipt-new.jsp"),
...
public class OnlineReceiptAction extends BaseFormAction {
	...
	private transient BillInfoImpl collDetails = new BillInfoImpl();
	...
	private transient PaymentResponse paymentResponse;
	...
	@Autowired
	private transient ChartOfAccountsHibernateDAO chartOfAccountsHibernateDAO;
	...
	public String saveNew() {
		/**
		 * initialise receipt info,persist receipt, create bill desk payment object and
		...
		if (null != paymentServiceId && paymentServiceId > 0)
			paymentService = (ServiceDetails) getPersistenceService()
					.findByNamedQuery(CollectionConstants.QUERY_SERVICE_BY_ID, paymentServiceId.longValue());
	...
	public String acceptMessageFromPaymentGateway() {
		...
		onlinePaymentReceiptHeader = receiptHeaderService.findByNamedQuery(
				CollectionConstants.QUERY_PENDING_RECEIPT_BY_ID_AND_CONSUMERCODE,
			...
			if (CollectionConstants.PGI_AUTHORISATION_CODE_SUCCESS.equals(paymentResponse.getAuthStatus()))
				processSuccessMsg();
					...
					|| (onlinePaymentReceiptHeader.getOnlinePayment().getService().getCode()
							...
							&& CollectionConstants.PGI_AUTHORISATION_CODE_PENDING
									.equals(paymentResponse.getAuthStatus()))) {
				...
				onlinePaymentReceiptHeader.getOnlinePayment()
						.setAuthorisationStatusCode(paymentResponse.getAuthStatus());
	 ...
	 * The receipt and the online transaction are both cancelled. The authorisation
	 * status for reason of failure is also persisted. The reason for payment
	 * failure is displayed back to the user
	 */
	private void processFailureMsg() {
		onlinePaymentReceiptHeader
				.setStatus(collectionsUtil.getReceiptStatusForCode(CollectionConstants.RECEIPT_STATUS_CODE_FAILED));
		...
		addActionError(getText(onlinePaymentReceiptHeader.getOnlinePayment().getService().getCode().toLowerCase()
				+ ".pgi." + paymentResponse.getAuthStatus()));
	 ...
	 * transaction status is changed from PENDING to SUCCCESS. The authorisation
	 * status for success(0300) for the online transaction is also persisted. An
	 * instrument of type 'ONLINE' is created with the transaction details and are
	 * persisted along with the receipt details. Voucher for the receipt is created
	 * and the Financial System is updated. The billing system is updated about the
	 * receipt creation. In case update to financial systems/billing system fails,
	 * the receipt creation is rolled back and the receipt/payment status continues
	 * to be in PENDING state ( and will be reconciled manually).
	 */
	...
	/**
	 * This method is invoked for manually reconciling online payments. If a payment
	 * is reconciled as a Success Payment, the receipt is created, the receipt is
	 * marked as APPROVED , the payment is marked as SUCCESS, and the voucher is
	 * created. If a payment is reconciled as To Be Refunded or Refunded, the
	 * transaction details are persisted, receipt is marked as FAILED and the
	 * payment is marked as TO BE REFUNDED/REFUNDED respectively. The billing system
	 * is updated about all the payments that have been successful.
	 *
	 * @return
	 */
	@SuppressWarnings("deprecation")
			...
			if (getStatusCode()[i].equals(CollectionConstants.ONLINEPAYMENT_STATUS_CODE_SUCCESS)) {
				...
				if (reconstructedList != null) {
					...
					debitAccountDetail = debitAccountHeadService.addDebitAccountHeadDetails(
							receipts[i].getTotalAmount(), receipts[i], BigDecimal.ZERO, receipts[i].getTotalAmount(),
	...
	public String viewReceipt() {
		...
		if (receiptHead != null) {
			...
			throw new ValidationException(
					Arrays.asList(new ValidationError("No Receipt Data Found", "No Receipt Data Found")));
	...
	public void prepare() {
		...
		// Fetching pending transaction by consumer code. If transaction is in pending
		...
		if (null != receiptHeader && isNotBlank(receiptHeader.getConsumerCode())
			...
			final List<ReceiptHeader> pendingOnlinePayments = getPersistenceService().findAllByNamedQuery(
					...
					receiptHeader.getService(), receiptHeader.getConsumerCode(),
					CollectionConstants.ONLINEPAYMENT_STATUS_CODE_PENDING);
	...
	private String decodeBillXML() {
		...
		try {
			ESAPI.encoder().decodeFromURL(getCollectXML());
	 ...
	 * configuration defined in AppConfig. Default is ALL, which means all payment
	 * gateway will appear for the billing service. Otherwise, only the payment
	 * gateway codes defined as comma separated values will appear for the Billing
	 * Service.
	 */
	@SuppressWarnings({ "unchecked", "deprecation" })
	private void constructServiceDetailsList() {
		...
		for (final AppConfigValues appConfigVal : appConfigValuesList) {
			...
			if (attr.equalsIgnoreCase(collDetails.getServiceCode())) {
				...
				final List<String> serviceCodes = Arrays.asList(attrVal.split(","));
	...
	/**
	 * This getter will be invoked by framework from UI. It returns the total amount
	 * of bill accounts that are present in the XML arriving from the billing system
	 *
	 * @return
	 */
	...
	/**
	 * This getter will be invoked by the framework from UI. It returns the amount
	 * payed by the citizen.
	 *
