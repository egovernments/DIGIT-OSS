...
public class MutationBillingSlabValidator {
    ...
    public void validateCreate(MutationBillingSlabReq billingSlabReq) {
    ...
    public void validateUpdate(MutationBillingSlabReq billingSlabReq) {
        Map<String, String> errorMap = new HashMap<>();
        validateIfTenantIdIsUnique(billingSlabReq, errorMap);
    ...
    public void validateIfTenantIdIsUnique(MutationBillingSlabReq billingSlabReq, Map<String, String> errorMap) {
        Set<String> tenantIds = billingSlabReq.getBillingSlab().parallelStream().map(MutationBillingSlab::getTenantId)
    ...
    public void checkIfBillingSlabsExist(MutationBillingSlabReq billingSlabReq, Map<String, String> errorMap) {
        List<String> ids = billingSlabReq.getBillingSlab().parallelStream().map(MutationBillingSlab::getId)
                .collect(Collectors.toList());
        MutationBillingSlabRes billingSlabRes = mutationBillingSlabService.searchBillingSlabs(billingSlabReq.getRequestInfo(),
                MutationBillingSlabSearchCriteria.builder().id(ids).tenantId(billingSlabReq.getBillingSlab().get(0).getTenantId()).build());
        if (CollectionUtils.isEmpty(billingSlabRes.getBillingSlab())) {
            ...
            List<String> idsAvailableintheDB = billingSlabRes.getBillingSlab().parallelStream().map(MutationBillingSlab::getId)
                    .collect(Collectors.toList());
     ...
     * @param errorMap
     */
    public void validateDuplicateBillingSlabs(MutationBillingSlabReq billingSlabReq, Map<String, String> errorMap) {

        List<MutationBillingSlab> incomingSlabs = billingSlabReq.getBillingSlab();
        String tenantId = incomingSlabs.get(0).getTenantId();
        List<Integer> errorList = new ArrayList<>();

        List<MutationBillingSlab> dbSlabs = mutationBillingSlabService.searchBillingSlabs(billingSlabReq.getRequestInfo(),
                MutationBillingSlabSearchCriteria.builder().tenantId(tenantId).build()).getBillingSlab();
...
/*					index = dbSlabs.indexOf(incomingSlabs.get(i));
    ...
    public void fetchAndvalidateMDMSCodes(MutationBillingSlabReq billingSlabReq, Map<String, String> errorMap) {
    ...
    public void validateMDMSCodes(MutationBillingSlabReq billingSlabReq, Map<String, String> errorMap, Object mdmsResponse) {
