...
public class DemandService {
	...
	public void generateDemand(RequestInfo requestInfo, List<Calculation> calculations) {
		...
		if (!CollectionUtils.isEmpty(calculations)) {
			...
			Set<String> applicationNos = calculations.stream()
					.map(calculation -> calculation.getFsm().getApplicationNo()).collect(Collectors.toSet());
	...
	private List<Demand> updateDemand(RequestInfo requestInfo, List<Calculation> calculations) {
		...
		for (Calculation calculation : calculations) {
			...
			List<Demand> searchResult = searchDemand(calculation.getTenantId(),
					Collections.singleton(calculation.getFsm().getApplicationNo()), requestInfo, calculation);

			if (CollectionUtils.isEmpty(searchResult))
				throw new CustomException(CalculatorConstants.INVALID_UPDATE,
						"No demand exists for applicationNumber: " + calculation.getFsm().getApplicationNo());
	...
	private List<Demand> createDemand(RequestInfo requestInfo, List<Calculation> calculations) {
		...
		for (Calculation calculation : calculations) {
			...
			if (calculation.getFsm() != null) {
				...
				fsm = fsmService.getFsmApplication(requestInfo, calculation.getTenantId(),
						calculation.getApplicationNumber());
			}

			if (fsm == null)
				throw new CustomException(CalculatorConstants.INVALID_APPLICATION_NUMBER,
						"Demand cannot be generated for applicationNumber " + calculation.getApplicationNumber()
								+ "  FSM application with this number does not exist ");
			...
			demands.add(Demand.builder().consumerCode(consumerCode).demandDetails(demandDetails).payer(owner)
					.minimumAmountPayable(minimumPayableAmt).tenantId(tenantId)
					.taxPeriodFrom(startCal.getTimeInMillis()).taxPeriodTo(endCal.getTimeInMillis()).consumerType("FSM")
					.businessService(utils.getBillingBusinessService(calculation.getFeeType())).build());
		}
	...
	private void addRoundOffTaxHead(String tenantId, List<DemandDetail> demandDetails) {
		...
		if (roundOff.compareTo(BigDecimal.ZERO) != 0) {
			DemandDetail roundOffDemandDetail = DemandDetail.builder().taxAmount(roundOff)
					.taxHeadMasterCode(CalculatorConstants.MDMS_ROUNDOFF_TAXHEAD).tenantId(tenantId)
					.collectionAmount(BigDecimal.ZERO).build();
