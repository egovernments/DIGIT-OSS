			...
			if (driver.getMobileNumber() != null) {
				...
				if (userDetailResponse != null && !CollectionUtils.isEmpty(userDetailResponse.getUser())) {
					...
					if (foundDriver == null ) {
							...
							foundDriver.getRoles().add(getRolObj(VendorConstants.DSO_DRIVER, VendorConstants.DSO_DRIVER_ROLE_NAME));
				...
				log.debug("MobileNo is not existed in Application.");
				errorMap.put(VendorErrorConstants.INVALID_DRIVER_ERROR, "MobileNo is mandatory for Driver " + driver.toString());
			}
		});
